...
__global__ void
wlb_shuffle_memcpy_kernel(uint8_t *gathered, uint8_t *shuffled, const int64_t *shard_lens,
                          const int64_t *shard_gathered_offsets, const size_t num_docs,
                          const size_t num_total_tokens, const size_t hidden_bytes) {
  ...
  // The global shard id w.r.t. shard_len.flatten()'s layout (i.e. (num_docs,
  // num_shards))
  ...
  // iterating in the shuffled layout because the gathered layout may have
  // padding.
  for (size_t token_id = blockIdx.x; token_id < num_total_tokens; token_id += gridDim.x) {
    ...
    uint8_t *gathered_token =
        gathered +
        (cur_shard_start_gathered_id + (token_id - cur_shard_start_token_id)) * hidden_bytes;
    uint8_t *shuffled_token = shuffled + token_id * hidden_bytes;
    for (size_t i = threadIdx.x; i * sizeof(int4) < hidden_bytes; i += blockDim.x) {
...
void launch_wlb_shuffle_memcpy(uint8_t *gathered, uint8_t *shuffled, const int64_t *shard_lens,
                               const int64_t *shard_gathered_offsets, const size_t num_docs,
                               const size_t num_total_tokens, const size_t hidden_bytes,
                               const bool is_forward, const cudaStream_t stream) {
  const int num_blocks = std::max((int)num_total_tokens / 32, 128);
  ...
  if (is_forward) {
    CUDACHECK(cudaLaunchKernel((void *)wlb_shuffle_memcpy_kernel<true>, dimGrid, dimBlock, args,
                               sharedMemory, stream));
  } else {
    CUDACHECK(cudaLaunchKernel((void *)wlb_shuffle_memcpy_kernel<false>, dimGrid, dimBlock, args,
                               sharedMemory, stream));
  }
}
}; // namespace wlb
