...
void wlb_shuffle_memcpy(at::Tensor &gathered_tensor, at::Tensor &shuffled_tensor,
                        const at::Tensor &shard_lens, const at::Tensor &shard_gathered_offsets,
  ...
  launch_wlb_shuffle_memcpy((uint8_t *)gathered_tensor.data_ptr(),
                            (uint8_t *)shuffled_tensor.data_ptr(), shard_lens.data_ptr<int64_t>(),
                            shard_gathered_offsets.data_ptr<int64_t>(), num_docs, num_total_tokens,
                            hidden_bytes, is_forward, stream);
}

}; // namespace

TORCH_LIBRARY(wlbmemcpy_kernels, m) { m.def("wlb_shuffle_memcpy", &wlb_shuffle_memcpy); }
