...
def compute_global_fwd_bwd(q, k, v, d_out, doc_lens, softmax_scale=None):
    ...
    _flash_attn_varlen_backward(
        ...
        dv,
        cu_seqlens_q,
        cu_seqlens_k,  # cu_seqlens_q / cu_seqlens_k
        int(max_seqlen_q),
        int(max_seqlen_k),  # max_seqlen_q / max_seqlen_k
        dropout_p=0.0,
        softmax_scale=softmax_scale,
        causal=True,
        window_size_left=-1,
...
def run(rank: int, world_size: int, args):
    ...
    if rank == 0:
        ...
        q_global = torch.empty(
            context_length, num_heads, head_dim, dtype=torch.bfloat16, device=device
        )
        k_global = torch.empty(
            context_length, num_heads, head_dim, dtype=torch.bfloat16, device=device
        )
        v_global = torch.empty(
            context_length, num_heads, head_dim, dtype=torch.bfloat16, device=device
        )
        d_out_global = torch.empty(
            context_length, num_heads, head_dim, dtype=torch.bfloat16, device=device
        )
    ...
    out_ref, dq_ref, dk_ref, dv_ref = compute_global_fwd_bwd(
        q_global, k_global, v_global, d_out_global, doc_lens, softmax_scale
    )
    dist.barrier(device_ids=[rank])
    ...
    per_seq_correctness_evaluate(
        dq_ref, local_q.grad, context_length, cp_size, rank, rtol=1e-1, atol=1e-1
    )
    per_seq_correctness_evaluate(
        dk_ref, local_k.grad, context_length, cp_size, rank, rtol=1e-1, atol=1e-1
    )
    per_seq_correctness_evaluate(
        dv_ref, local_v.grad, context_length, cp_size, rank, rtol=1e-1, atol=1e-1
    )
