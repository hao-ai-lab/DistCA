...
from wlbllm.utils import compute_per_doc_cp_shard_doc_len, generate_doc_lens
...
def random_tensor_generation(batch_size, context_length, num_heads, head_dim, device, gqa=1):
    ...
    q_global = torch.randn(
        batch_size * context_length, num_heads, head_dim, device=device, dtype=torch.bfloat16
    )
    k_global = torch.randn(
        batch_size * context_length, num_kv_heads, head_dim, device=device, dtype=torch.bfloat16
    )
    v_global = torch.randn(
        batch_size * context_length, num_kv_heads, head_dim, device=device, dtype=torch.bfloat16
    )
    d_out_global = torch.randn(
        batch_size * context_length, num_heads, head_dim, device=device, dtype=torch.bfloat16
    )
...
def compute_per_seq_metadate_chunk(
...
def compute_per_doc_metadate_chunk(
...
def run(rank: int, world_size: int, args, return_dict):
    ...
    q_global, k_global, v_global, d_out_global = random_tensor_generation(
        batch_size, context_length, num_heads, head_dim, device, gqa=args.gqa
    )
    (
        local_q,
        local_k,
        ...
        k_offsets,
        local_d_out,
    ) = compute_per_seq_metadate_chunk(
        context_length, q_global, k_global, v_global, doc_lens, cp_size, rank, d_out=d_out_global
    )
    ...
    for _ in range(n_iter):
        perseq_allgather_events.append(
            [torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)]
        )
        perseq_allreduce_events.append(
            [torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)]
        )
        perseq_attn_events.append(
            [torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)]
        )
    ...
    (
        ...
        kv_idx_list,
        local_d_out,
    ) = compute_per_doc_metadate_chunk(
        context_length,
        q_global,
        k_global,
        v_global,
    ...
    perdoc_attn_events = []
    for _ in range(n_iter):
        perdoc_allgather_events.append(
            [torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)]
        )
        perdoc_allreduce_events.append(
            [torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)]
        )
        perdoc_attn_events.append(
            [torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)]
        )
    # warmup:
    for _ in range(n_warmup):
    ...
    print(
        f"rank:{rank}, per_seq_latency:{per_seq_latency:.3f}ms, per_doc_latency:{per_doc_latency:.3f}ms, speedup:{speedup:.3f}x"
    )
    ...
    for allgather_event in perseq_allgather_events:
        allgather_event[0].wait()
        allgather_event[1].wait()
