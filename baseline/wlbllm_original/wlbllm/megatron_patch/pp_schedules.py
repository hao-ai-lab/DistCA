...
_DEBUG_PRINT_ENABLED = (
    os.environ.get("WLB_DEBUG", "0") == "1" or os.environ.get("WLBLLM_DEBUG", "0") == "1"
)
...
def recv_forward(tensor_shapes: list, config: dict) -> list[torch.Tensor]:
    debug_print("  🟡 recv_forward start: %s", lambda: tensor_shapes)
...
def send_forward(
    output_tensor: list[torch.Tensor], send_tensor_shapes: list, config: dict
) -> list[torch.Tensor]:
...
def send_backward(
    input_tensor_grad: list[torch.Tensor] | None, recv_tensor_shapes: list | None, config: dict
) -> list[torch.Tensor]:
...
def send_forward_recv_backward(
    ...
    assert send_tensor_shapes is not None, (
        "send_tensor_shapes should not be None in send_forward_recv_backward function"
    )
...
def send_backward_recv_forward(
    input_tensor_grad: list[torch.Tensor], recv_tensor_shapes: list, config: dict
) -> list[torch.Tensor]:
    assert recv_tensor_shapes is not None, (
        "recv_tensor_shapes should not be None in send_backward_recv_forward function"
    )
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    if is_wlb_func():
        print("🟡 forward_backward_pipelining_without_interleaving: is_wlb_func()")
    ...
    def forward_step__with_nvtx(*args, **kwargs):
        ...
        forward_batch_id += 1
        debug_print(
            "      🟡 print_shapes_of_tensor_list(ret[0]) = %s",
            lambda: print_shapes_of_tensor_list(ret[0]),
        )
        return ret
    ...
    print(
        f"🟡[pp_rank = {rank}] num_warmup_microbatches: {num_warmup_microbatches}, num_microbatches_remaining: {num_microbatches_remaining}"
    )
    ...
    for i in range(num_warmup_microbatches):
        ...
        recv_tensor_shapes = [
            get_microbatch_tensor_size(forward_batch_id, micro_batch_size, config)
        ]
    ...
    if num_microbatches_remaining > 0:
        print("🟡 forward_backward_pipelining_without_interleaving[remaining]: start recv forward")
        recv_tensor_shapes = [
            get_microbatch_tensor_size(forward_batch_id, micro_batch_size, config)
        ]
        input_tensor = recv_forward(recv_tensor_shapes, config)
        print("🟡 forward_backward_pipelining_without_interleaving[remaining]: end recv forward")

    assert input_tensor is not None, "input_tensor should not be None before entering steady state"
    ...
    for i in range(num_microbatches_remaining):
        ...
        if forward_only:
            raise NotImplementedError(
                "forward_only is not supported in wlbllm. It should be simple: Just replace the forwrad index function with integer index."
            )
            print(
                f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: start send forward"
            )
            ...
            if not last_iteration:
                print(
                    f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: start recv forward"
                )
                input_tensor = recv_forward(recv_tensor_shapes, config)
                print(
                    f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: end recv forward"
                )

        else:
            print(
                f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: start send forward recv backward"
            )

            backward_tensor_shapes = [
                get_microbatch_tensor_size(backward_batch_id, micro_batch_size, config)
            ]  # this should actually be the backward shape
            output_tensor_grad = send_forward_recv_backward(
                output_tensor, backward_tensor_shapes, config
            )
            print(
                f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: end send forward recv backward"
            )
            ...
            print(
                f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: start backward step"
            )
            input_tensor_grad = modified_backward_step(
                input_tensor, output_tensor, output_tensor_grad, model_type, config
            )
            print(
                f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: end backward step"
            )

            if last_iteration:
                input_tensor = None
                print(
                    f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: start send backward"
                )
                send_backward(input_tensor_grad, None, config)
                print(
                    f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: end send backward"
                )
            else:
                print(
                    f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: start send backward recv forward"
                )
                backward_tensor_shapes = [
                    get_microbatch_tensor_size(backward_batch_id, micro_batch_size, config)
                ]
                ...
                print(
                    f"🟡 forward_backward_pipelining_without_interleaving[1F1B{i}]: end send backward recv forward"
                )
    ...
    if not forward_only:
        for i in range(num_warmup_microbatches):
            ...
            print(
                f"🟡 forward_backward_pipelining_without_interleaving[cooldown{i}]: start backward step"
            )
            input_tensor = input_tensors.pop(0)
            output_tensor = output_tensors.pop(0)
            ...
            print(
                f"🟡 forward_backward_pipelining_without_interleaving[cooldown{i}]: end backward step"
            )

            print(
                f"🟡 forward_backward_pipelining_without_interleaving[cooldown{i}]: start send backward"
            )
            send_backward(input_tensor_grad, None, config)
            print(
                f"🟡 forward_backward_pipelining_without_interleaving[cooldown{i}]: end send backward"
            )
