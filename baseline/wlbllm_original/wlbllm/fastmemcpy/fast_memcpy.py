...
def prepare_metadata(
    doc_shards: list[list[doc_shard]], device: torch.device, cp_size: int, chunk_size: int
):
    assert len(doc_shards) == cp_size * 2
    # shape (num_chunks, num_docs)
    chunk_shard_lens = torch.tensor(
        [[ds.shard_len if ds is not None else 0 for ds in cds] for cds in doc_shards],
        dtype=torch.int64,
        device=device,
    )
...
def gathered_shuffled_switch(
    ...
    shard_gathered_offsets: torch.Tensor,
    is_from_gathered: bool,
):
    _ops.wlb_shuffle_memcpy(
        gathered_tensor, shuffled_tensor, shard_lens, shard_gathered_offsets, is_from_gathered
    )


def _shuffle_per_doc_cp(
    context_length: int,
    tensor_list,
    cp_size: int,
    chunk_shard_lens: torch.Tensor,
    shard_src_offset: torch.Tensor,
    num_tokens: int,
):
    ...
    shuffled_tensor = torch.empty(
        out_shape, dtype=gathered_tensor.dtype, device=gathered_tensor.device
    )
    shuffled_tensor = shuffled_tensor.flatten(start_dim=1).contiguous()

    gathered_shuffled_switch(
        gathered_tensor, shuffled_tensor, chunk_shard_lens, shard_src_offset, is_from_gathered=True
    )
...
def kv_shuffle_for_per_doc_cp_fast(
    ...
    k_global = _shuffle_per_doc_cp(
        context_length, k_tensor_list, cp_size, chunk_shard_lens, shard_src_offset, num_tokens
    )
    if v_tensor_list is not None:
        v_global = _shuffle_per_doc_cp(
            context_length, v_tensor_list, cp_size, chunk_shard_lens, shard_src_offset, num_tokens
        )
    else:
        v_global = None
...
def _unshuffle_per_doc_cp(
    context_length: int,
    grad_shuffled: torch.Tensor,
    cp_size: int,
    chunk_shard_lens: torch.Tensor,
    shard_src_offset: torch.Tensor,
):
    ...
    gathered_tensor = torch.empty(
        gathered_tensor_shape, dtype=grad_shuffled.dtype, device=grad_shuffled.device
    )
    grad_shuffled = grad_shuffled.flatten(start_dim=1).contiguous()
    gathered_tensor = gathered_tensor.flatten(start_dim=2).contiguous()
    gathered_shuffled_switch(
        gathered_tensor, grad_shuffled, chunk_shard_lens, shard_src_offset, is_from_gathered=False
    )
...
def kv_unshuffle_for_per_doc_cp_fast(
    context_length, grad_k, grad_v, cp_size, chunk_shard_lens, shard_src_offset
):
    grad_k_list = _unshuffle_per_doc_cp(
        context_length, grad_k, cp_size, chunk_shard_lens, shard_src_offset
    )
    if grad_v is not None:
        grad_v_list = _unshuffle_per_doc_cp(
            context_length, grad_v, cp_size, chunk_shard_lens, shard_src_offset
        )
    else:
        grad_v_list = None
