...
def setup_batches(max_seq_len: int = 4096):
    ...
    for idx, token_lens in enumerate(batches):
        ...
        for batch_id, token_len in enumerate(token_lens):
...
global_batches = setup_batches()


from context_parallel import (
    get_batch_on_this_cp_rank__balanced_seq,
...
def get_batch(data_iterator):
    ...
    for k, v in batch.items():
        print(k, v.shape)
    """
    batch.values() == (tokens, labels, loss_mask, attention_mask, position_ids)
    """
    return batch.values(), token_lens
...
def train_valid_test_datasets_provider(train_val_test_num_samples):
    ...
    if args.mock_data:
        # <<< (GindaChen)(Hack) Possibly inject a mock dataset here >>>
