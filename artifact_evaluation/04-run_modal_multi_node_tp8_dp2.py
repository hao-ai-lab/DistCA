...
image = (
    ...
    .env({
        "DISTCA_ROOT": "/workspace/DistCA",
        "DISTCA_SMOKE_MODEL": "/workspace/DistCA/models/tiny-smoke-llama8",
    })
...
CLUSTER_SIZE = int(os.environ.get("DISTCA_CLUSTER_SIZE", "1"))
GPU_TYPE = os.environ.get("DISTCA_GPU_TYPE", "A100")

if CLUSTER_SIZE > 1:
    cluster_decorator = modal.experimental.clustered(size=CLUSTER_SIZE, rdma=True)
else:
    def cluster_decorator(fn):
        return fn
...
def run_gpu_llama_multi_node():
    ...
    try:
        ...
        # Non-clustered fallback: behave like a single-node job.
        rank = 0
        world_size = 1
        master_ip = "127.0.0.1"
    ...
    sed -i 's/--nproc_per_node=1/--nproc_per_node={effective_gpus_per_node}/g' /workspace/DistCA/scripts/multi_node_smoke.sh
    ...
    sed -i 's/--num-gpus-per-node 1/--num-gpus-per-node {effective_gpus_per_node}/g' /workspace/DistCA/scripts/multi_node_smoke.sh
    sed -i 's/--num-nodes 1/--num-nodes {world_size}/g' /workspace/DistCA/scripts/multi_node_smoke.sh
    sed -i 's/--tp-size 1/--tp-size {effective_tp_size}/g' /workspace/DistCA/scripts/multi_node_smoke.sh
    sed -i 's/--max-sample-id 1/--max-sample-id 100/g' /workspace/DistCA/scripts/multi_node_smoke.sh
    sed -i 's/--num-layers 1/--num-layers {effective_num_layers}/g' /workspace/DistCA/scripts/multi_node_smoke.sh
