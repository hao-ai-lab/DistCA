...
from ..inplace_metadata import compute_dst_offsets, compute_reverse_comm
...
class Worker:
    ...
    def test(self, seed: int, tensor, num_received,
             dst_id, dst_offset,
             rev_dst_id, rev_dst_offset):
        ...
        dst_tensor = self.orchestrate(tensor, dst_id, dst_offset, dst_tensor=dst_tensor)
        back_tensor = self.orchestrate(dst_tensor, rev_dst_id, rev_dst_offset, dst_tensor=back_tensor)
    ...
    def orchestrate(self, tensor: torch.Tensor, dst_id: torch.Tensor, dst_offset: torch.Tensor, dst_tensor: torch.Tensor):
        dispatch(dst_tensor, None, tensor, None, dst_id, dst_offset, None, None)
        nvshmem_barrier_all()
...
def create_testcase(seed: int, world_size: int, num_tokens: int, cp_degree: int):
    torch.manual_seed(seed)
    dst_id = torch.randint(-1, world_size, (world_size, num_tokens, cp_degree))
    ...
    back_tensor_dedup = back_tensor.sum(dim=2) / (back_tensor != 0).sum(dim=2)
    return dst_id, dst_offset, rev_dst_id, rev_dst_offset, dst_tensor, back_tensor_dedup

def test(seed, world_size, num_tokens, cp_degree, workers):
    dst_id, dst_offset, rev_dst_id, rev_dst_offset, dst_tensor, back_tensor_dedup = create_testcase(seed, world_size, num_tokens, cp_degree)
    assert len(workers) == world_size
    refs = []

    one_hot_dest = torch.nn.functional.one_hot(
        dst_id.reshape(world_size, -1) + 1, num_classes=world_size + 1
    ).long()[:, :, 1:]
    num_received = torch.sum(one_hot_dest, dim=(0, 1))
    ...
    for rank, worker in enumerate(workers):
        ...
        num_received_slice = num_received[rank]
        ref = worker.test.remote(
            seed, tensor_slice, num_received_slice,
            dst_id_slice, dst_offset_slice, rev_dst_id_slice, rev_dst_offset_slice
        )
        refs.append(ref)
