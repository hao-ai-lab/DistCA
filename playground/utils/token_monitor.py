...
def set_token_monitor_config(
    ...
    max_tokens_to_decode: int | None = None,
    max_samples_to_log: int | None = None,
    max_docs_to_preview: int | None = None,
    doc_preview_chars: int | None = None,
    preview_edge_chars: int | None = None,
) -> None:
...
def analyze_position_ids_for_doc(
    ...
    expected = torch.arange(
        min_pos, min_pos + len(doc_pos), device=doc_pos.device, dtype=doc_pos.dtype
    )
...
def describe_attention_mask(
    ...
    causal_float = torch.tril(
        torch.ones(seq_len, seq_len, device=mask_2d.device, dtype=torch.float32)
    )
...
def log_tokens_text(
    ...
    config: TokenMonitorConfig | None = None,
) -> None:
    ...
    log.info(
        f"=== Iteration {iter_num} Token Monitor (batch_size={batch_size}, seq_len={seq_length}, eod_id={eod_token_id}) ==="
    )
    ...
    for sample_idx in range(batch_size):
        ...
        if sample_idx < cfg.max_samples_to_log:
            ...
            try:
                ...
                log.info(
                    f"    Tokens per doc: {doc_info.tokens_per_doc[:10]}{'...' if len(doc_info.tokens_per_doc) > 10 else ''}"
                )
                log.info(
                    f"    EOD positions: {doc_info.eod_positions[:10]}{'...' if len(doc_info.eod_positions) > 10 else ''}"
                )
                log.info(
                    f"    Token IDs (first 20): {truncated_tokens[:20]}{'...' if len(truncated_tokens) > 20 else ''}"
                )
                log.info(f"    Text (first {len(truncated_tokens)} tokens): {repr(decoded_text)}")

                # Calculate document start/end positions
                doc_starts = [0] + [
                    pos + 1 for pos in doc_info.eod_positions if pos + 1 < len(full_sample_tokens)
                ]
                doc_ends = [pos + 1 for pos in doc_info.eod_positions] + (
                    [len(full_sample_tokens)]
                    if doc_info.eod_positions
                    and doc_info.eod_positions[-1] + 1 < len(full_sample_tokens)
                    else []
                )
                ...
                for doc_idx, start_pos in enumerate(doc_starts[: cfg.max_docs_to_preview]):
                    end_pos = (
                        doc_ends[doc_idx] if doc_idx < len(doc_ends) else len(full_sample_tokens)
                    )
                    ...
                    if sample_loss_mask is not None:
                        ...
                        info_parts.append(
                            f"loss_mask: {loss_info['ones']}/{loss_info['total']} active ({loss_info['loss_ratio']:.1%})"
                        )

                    # Position IDs info
                    if sample_position_ids is not None:
                        pos_info = analyze_position_ids_for_doc(
                            sample_position_ids, start_pos, end_pos
                        )
                ...
                if len(doc_starts) > cfg.max_docs_to_preview:
                    log.info(
                        f"      ... and {len(doc_starts) - cfg.max_docs_to_preview} more documents"
                    )
    ...
    if all_doc_lengths:
        ...
        if loss_mask is not None:
            ...
            log.info(
                f"    Loss mask: {total_ones}/{total_elements} active ({total_ones / total_elements:.1%})"
            )
