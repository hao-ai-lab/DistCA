...
class MegatronModelArgs:
    ...
    num_query_groups: int | None = None  # For GQA, None means same as num_attention_heads
    ffn_hidden_size: int | None = None  # None means 4 * hidden_size
    seq_length: int = 4096
...
def get_hf_config(
    model_name_or_path: str, trust_remote_code: bool = True, cache_dir: str | None = None
) -> "PretrainedConfig":
    """
...
def hf_config_to_megatron_args(
    ...
    num_layers = (
        num_layers_override if num_layers_override is not None else hf_config.num_hidden_layers
    )
...
def get_megatron_args_dict_from_hf_model(
    ...
    seq_length: int | None = None,
    num_layers_override: int | None = None,
    max_position_embeddings_override: int | None = None,
    trust_remote_code: bool = True,
    cache_dir: str | None = None,
) -> dict[str, Any]:
    """
    ...
    if max_position_embeddings_override is not None:
        ...
        logger.info(
            f"Overriding hf_config.max_position_embeddings to {max_position_embeddings_override}"
        )
        ...
        logger.info(
            f"Setting hf_config.max_position_embeddings to {seq_length} (matching seq_length)"
        )
        logger.info(
            "  To train with seq_length < max_position_embeddings, set max_position_embeddings_override "
            "to the desired value (e.g., model's original max_position_embeddings)"
        )
