...
def _format_tensor_stats(stats, prefix=""):
    ...
    if "nan_count" in stats and "inf_count" in stats:
        ...
        lines.append(
            f"{prefix}nan_count={stats['nan_count']}, inf_count={stats['inf_count']}{zero_info}"
        )
...
def register_nan_detection_hooks(
                ...
                for i, inp in enumerate(input):
                    if isinstance(inp, torch.Tensor):
                        if torch.isnan(inp).any():
                            ...
                            logger.error(_colorize_text(msg, _COLOR_RED))
                            if log_tensor_stats:
                                stats = _get_tensor_stats(
                                    inp,
                                    f"INPUT[{i}]",
                                    show_start_end=True,
                                    num_elements=tensor_stats_elements,
                                )
            ...
            if isinstance(output, torch.Tensor):
                    ...
                    if check_inputs and len(input) > 0:
                        for i, inp in enumerate(input):
                            if isinstance(inp, torch.Tensor):
                                ...
                                inp_zero_ratio = (
                                    inp_zero_count / inp.numel() if inp.numel() > 0 else 0.0
                                )
                                msg = (
                ...
                if torch.isnan(output).any():
                    ...
                    if log_tensor_stats:
                        ...
                        msg = (
                            f"[OK] OUTPUT from {name} ({module_name}): shape={output.shape}, no NaN"
                        )
                        logger.info(_colorize_text(msg, _COLOR_GREEN))
    ...
    if isinstance(unwrapped_model, list):
        logger.info(
            f"Model is a list with {len(unwrapped_model)} modules, registering hooks on all"
        )
