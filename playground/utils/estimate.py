...
def get_gpu_peak_tflops(
    device_id: int = 0, logger: logging.Logger | None = None
) -> tuple[str, float]:
...
def log_memory_estimate(
    ...
    log.info("=== Theoretical Memory Report ===")
    ...
    log.info(
        "  Note: Activation memory is only accurate with sequence_parallel + selective recompute"
    )
    log.info("=================================")
...
def log_flops_estimate(
    ...
    estimate = estimate_flops(args, num_microbatches, micro_batch_size, dp_world_size, tp, pp, cp)

    gpu_name, gpu_peak_tflops = get_gpu_peak_tflops(device_id, logger=log)

    log.info("=== Theoretical FLOPS Report ===")
    log.info(
        f"  Batch size config: micro_batch={estimate.micro_batch_size}, "
        f"num_microbatches={estimate.num_microbatches}, "
        f"global_batch={estimate.global_batch_size}"
    )
    log.info(
        f"  Parallelism: TP={estimate.tp}, PP={estimate.pp}, CP={estimate.cp}, DP={estimate.dp}"
    )
    log.info(
        f"  FLOPs per forward pass (1 micro-batch): "
        f"{estimate.flops_per_forward_pass:.2e} ({estimate.tflops_per_forward:.2f} TFLOP)"
    )
    log.info(
        f"  FLOPs per iteration per GPU (fwd+bwd): "
        f"{estimate.flops_per_iteration_per_gpu:.2e} ({estimate.tflops_per_iteration_per_gpu:.2f} TFLOP)"
    )
    log.info(
        f"  Total FLOPs per iteration (all GPUs): "
    ...
    log.info(
        f"  To compute MFU at runtime: MFU = (TFLOP per iteration / time_sec) / {gpu_peak_tflops:.1f}"
    )
    log.info("================================")
