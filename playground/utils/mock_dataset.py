    ...
    def summary(self) -> str:
        ...
        for dp_rank in range(self.dp_size):
            ...
            for mb_idx in range(self.num_microbatches):
                ...
                lines.append(
                    f"    Microbatch {mb_idx}: {len(mb.samples)} samples, docs_per_sample={doc_lens_per_sample}"
                )
...
class GlobalBatchPeeker:
    ...
    def __init__(
        ...
        self.samples_per_global_batch = self.micro_batch_size * self.num_microbatches * self.dp_size

        logger.info("GlobalBatchPeeker initialized:")
    ...
    def _get_sample_info(self, sample_idx: int) -> SampleInfo:
        ...
        if hasattr(self.dataset, "_query_document_sample_shuffle_indices"):
            ...
            if len(document_ids) == 0:
                ...
                documents.append(DocumentInfo(doc_id=-1, start_offset=0, length=len(text)))
            else:
                ...
                for i, doc_id in enumerate(document_ids):
                    ...
                    documents.append(
                        DocumentInfo(doc_id=doc_id, start_offset=start, length=end - start)
                    )
                    start = end

            return SampleInfo(sample_idx=sample_idx, total_tokens=len(text), documents=documents)
        else:
    ...
    def peek_iteration(self, iteration: int = 0) -> GlobalBatchLayout:
        ...
        for dp_rank in range(self.dp_size):
            for mb_idx in range(self.num_microbatches):
                ...
                microbatches.append(
                    MicrobatchInfo(microbatch_idx=mb_idx, dp_rank=dp_rank, samples=samples)
                )
