...
def explore_configurations(
    ...
    df = df.sort_values(by=["Tokens/Step", "Total (GB)"], ascending=[False, True])
    return df
...
def render_sidebar():
    ...
    return {
        ...
        "use_distributed_optimizer": use_distributed_opt
        if "use_distributed_opt" in dir()
        else True,
...
def render_main_content(config: dict[str, Any]):
    ...
    st.markdown(
        '<p class="subtitle">Find optimal parallelism configurations for your GPU cluster</p>',
        unsafe_allow_html=True,
    )
    ...
    with col3:
        st.metric(
            ...
            f"{config['num_gpus'] * config['gpu'].memory_gb:.0f} GB total",
        )
    with col4:
        total_configs = (
    ...
    if "results_df" in st.session_state:
        ...
        if df.empty:
            ...
            styled_df = df_display.style.applymap(highlight_memory, subset=["Memory %"]).format(
                {
                    "Weight (GB)": "{:.2f}",
                    "Optimizer (GB)": "{:.2f}",
                    "Activation (GB)": "{:.2f}",
