...
class SimpleBatchGenerator:
    ...
    def get_next_batch(self, num_batches: int) -> list[list[int]]:
        ...
        for _ in range(num_batches):
            ...
            doc_lens = create_list(num_docs_in_batch, self.total_token_on_rank, self.tp_size, 1)
...
def create_pipeline_doclens(
    ...
    use_planner: bool = False,
    batch_generator: SimpleBatchGenerator | None = None,
    ...
    if add_dummy:
        ...
        if use_planner and batch_generator is not None:
            ...
            debug_print("In create_pipeline_doclens, before calling get_next_batch")
    ...
    debug_print(
        f"In create_pipeline_doclens, after calling get_next_batch: pp_head_new_doc_len is: {pp_head_new_doc_len}"
    )
    ...
    debug_print(
        f"In create_pipeline_doclens, finally tick_per_rank_doc_len is: {tick_per_rank_doc_len}"
    )

    if return_original_doclen:
        debug_print(
            "In create_pipeline_doclens, return_original_doclen is True, returning (tick_per_rank_doc_len, pp_head_new_doc_len)"
        )
...
def create_single_microbatch_set(
    config: MicrobatchConfig, return_seq_lens: bool = False
    ...
    print(
        f"Creating {num_ticks} microbatches (num_microbatch={config.num_microbatch}, "
    ...
    for tick_idx in range(num_ticks):
        ...
        bwd_packed_seq_params = PackedSeqParams(qkv_format="thd", **fa_bwd_params[config.as_rank])
        ...
        tensor_doc_lens = torch.tensor(
            tick_per_rank_doc_lens_mlp[config.as_rank], dtype=torch.int32
        )
    ...
    for microbatch_idx, microbatch in enumerate(microbatches):
        ...
        bwd_metadata_idx = (microbatch_idx + config.pp_degree - 1 - pp_rank * 2) % len(bwd_metadata)
...
def combine_microbatches_for_ping_pong(
    microbatches_0: list[dict], microbatches_1: list[dict], seed: int | None = None
) -> list[dict]:
    """
...
def create_ping_pong_microbatches(
    config: MicrobatchConfig, seed: int | None = None, return_seq_lens: bool = False
) -> tuple[list[dict], list | None]:
    """
    ...
    microbatches_0, seq_lens_0 = create_single_microbatch_set(
        config, return_seq_lens=return_seq_lens
    )
    ...
    combined_microbatches = combine_microbatches_for_ping_pong(
        microbatches_0, microbatches_1, seed=seed
    )
...
def example_with_batch_generator():
    ...
    print("\nConfiguration:")
    print(f"  world_size={world_size}, dp_size={dp_size}, pp_degree={pp_degree}")
    print("  Using batch generator with seed=42")
