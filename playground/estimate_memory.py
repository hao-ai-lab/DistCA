...
class ParallelConfig:
    ...
    world_size: int | None = None  # Total GPUs (optional, used to infer DP)
...
def compute_weight_and_optimizer_memory(
    ...
    # Total per parameter
    total_bytes_per_param = (
        weight_bytes_per_param + optimizer_bytes_per_param + grad_bytes_per_param
    )
...
def estimate_memory_batch(
    ...
    configs: list[dict | ParallelConfig],
) -> list[MemoryEstimate]:
    """
...
Available models: """
    ...
    parser.add_argument(
        "--model", type=str, default="llama-8b", help="Model name (e.g., llama-8b, llama-70b)"
    )
    parser.add_argument(
        "--list-models", action="store_true", help="List available model configurations"
    )
    ...
    parser.add_argument(
        "--configs-file", type=str, default=None, help="Path to JSON file with config list"
    )

    # Sweep mode
    parser.add_argument(
        "--sweep", action="store_true", help="Run a sweep over common configurations"
    )
