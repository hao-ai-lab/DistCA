...
def pre_a2a_qkv(
    ...
    k: Tensor,
    v: Tensor,
    kv_dispatch_mask: Tensor,
    q_seq_tokens: Tensor,
    k_seq_tokens: Tensor,
    q_send_buffer_offset: Tensor,
    k_send_buffer_offset: Tensor,
    v_send_buffer_offset: Tensor,
    is_fwd: bool,
...
def post_a2a_qkv(
    ...
    recv_v: Tensor,
    kv_dispatch_mask: Tensor,
    q_recv_seq_tokens: Tensor,
    k_recv_seq_tokens: Tensor,
    q_recv_buffer_offset: Tensor,
    k_recv_buffer_offset: Tensor,
    v_recv_buffer_offset: Tensor,
    is_fwd: bool,
...
def post_a2a_attn_out_grad_resend_qkv(
    recv_attn_out_shape: list[int],
    recv_lse_shape: list[int],
    recv_q_shape: list[int],
    ...
    recv_merged_q = recv_k.new_empty((recv_attn_out_shape[0], merged_q_bytes), dtype=torch.uint8)
    ...
    tensors = torch.split(recv_merged_q, [*recv_q_bytes, pad_bytes], dim=1)[:-1]
    recv_attn_out_grad, recv_attn_out, recv_lse, recv_q = [
        t.view(dtype).contiguous() for t, dtype in zip(tensors, recv_q_dtypes)
    ]
    return (recv_attn_out_grad, recv_attn_out, recv_lse, recv_q, recv_k, recv_v)
