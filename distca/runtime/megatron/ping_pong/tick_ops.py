...
from distca.runtime.megatron.ops import FusedCommAttn, post_a2a_attn_out_with_lse
from distca.runtime.megatron.ops.fused_comm_attn import FlashAttnArgs
from distca.runtime.megatron.ops.stream_sync_fn import tick_sync_with_info
...
def forward_core_attn(layer: TransformerLayer, args: dict[str, Any]):
    ...
    if bwd_resend_qkv:
        ...
        signal = FusedCommAttn.apply(
            ...
            FlashAttnArgs(
                num_heads_q=layer.config.num_attention_heads
                // layer.config.tensor_model_parallel_size,
                num_heads_kv=layer.config.num_query_groups
                // layer.config.tensor_model_parallel_size,
...
def forward_post_then_pre_core_attn_cuda_graph(layer: TransformerLayer, args: dict[str, Any]):
    ...
    log_memory_usage(
        f"(L{layer.layer_number}) forward_post_then_pre_core_attn:(before non core attn)"
    )
    _log_tensor_shapes(
        layer,
    ...
    log_memory_usage(
        f"(L{layer.layer_number}) forward_post_then_pre_core_attn:(after non core attn)"
    )
    _log_tensor_shapes(
        layer,
...
def forward_post_core_attn_cuda_graph(layer: TransformerLayer, args: dict[str, Any]):
    ...
    _log_tensor_shapes(
        layer, "post_cuda_graph.post__forward_post_attn_cuda_graph", hidden_states=mlp_output
    )
    args["hidden_states"] = mlp_output
    log_memory_usage(f"(L{layer.layer_number}) forward_post_core_attn:(end)")
    return args
...
def tick_sync(
    ...
    keys_1,
    layer_info="unknown",
    operation_info="unknown",
):
    log_memory_usage("(L?) tick_sync:(start)")
    ...
    out_tensors = tick_sync_with_info(
        compute_stream, comm_stream, layer_info, operation_info, *tensors
    )
    ...
    log_memory_usage("(L?) tick_sync:(end)")


def tick_nonca_compute(
    layer: TransformerLayer,
    prev_layer: TransformerLayer | None,
    arg_group: dict[str, Any],
    is_last_layer_post_attn: bool,
):
    """
...
def _record_tick_timing(
    operation: str, layer_number: int, start_event: torch.cuda.Event, end_event: torch.cuda.Event
):
    """Record timing for a tick operation."""
    if _current_tick_sample_id is not None:
        _pending_tick_events.append(
            (_current_tick_sample_id, operation, layer_number, start_event, end_event)
        )
...
def sync_and_collect_tick_timing():
    ...
    for sample_id, operation, layer_number, start_event, end_event in _pending_tick_events:
        ...
        if sample_id not in _tick_times:
            _tick_times[sample_id] = {"forward_pre_core_attn": [], "forward_post_core_attn": []}

        # Store timing data
        if operation in _tick_times[sample_id]:
            _tick_times[sample_id][operation].append(
                {"layer_number": layer_number, "duration_ms": duration_ms}
            )
