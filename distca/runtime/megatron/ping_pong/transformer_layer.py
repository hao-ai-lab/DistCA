...
from distca.runtime.dispatch_fn import (
    all_to_all,
    post_all2all_layout_transfer,
    pre_all2all_layout_transfer,
)
from distca.runtime.megatron.base_transformer_layer import TransformerLayer as BaseTransformerLayer
from distca.runtime.megatron.ops import FusedCommAttn, TickSync, post_a2a_attn_out_with_lse
...
from distca.runtime.megatron.ping_pong.utils import gather_tensor, repack_args, splits_all
...
class TransformerLayer(BaseTransformerLayer):
    ...
    def ping_pong_forward(
        ...
        *,
        ...
        needs_gather = (
            debug or packed_seq_params.do_gather
        )  # NOTE: cannot infer if this is the last local layer or not.
        ...
        if debug:
            packed_seq_params_0.stream = compute_stream
            packed_seq_params_1.stream = compute_stream
        else:
        ...
        torch.cuda.nvtx.range_pop()
        # pre-communicate,0
        signal_0 = self._pre_mlp_to_attn(query_0, key_0, value_0, packed_seq_params_0)
        ...
        torch.cuda.nvtx.range_pop()
        # pre-communicate,1
        signal_1 = self._pre_mlp_to_attn(query_1, key_1, value_1, packed_seq_params_1)
        ...
        ## compute
        # post-communicate,0
        query_0, key_0, value_0 = self._post_mlp_to_attn(signal_0, packed_seq_params_0)
        ...
        signal_1, signal_0 = TickSync.apply(compute_stream, comm_stream, signal_1, signal_0)
        # NOTE: do not remove this debug tensor. see above.
    ...
    def forward_ping_pong_single_sided(
        ...
        *,
        ...
        if rotary_pos_emb is not None:
            rotary_pos_emb = None
            warnings.warn(
                "forward_ping_pong_single_sided is only to debug a single ping-pong "
        ...
        if backward_resend_qkv:
            signal = FusedCommAttn.apply(
                ...
                FlashAttnArgs(
                    ...
                    num_heads_kv=self.config.num_query_groups
                    // self.config.tensor_model_parallel_size,
                    head_dim=self.config.hidden_size // self.config.num_attention_heads,
