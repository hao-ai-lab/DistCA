...
from distca.runtime.megatron.ops.fused_comm_attn import dummy_backward
...
from distca.runtime.megatron.ping_pong.utils import gather_tensor, split_all_dict
...
class PingPongTransformerBlockInterface(MegatronTransformerBlock):
    ...
    def init_value(self):
        ...
        self.tick_nonca_compute = (
            tick_nonca_compute_cuda_graph if self.use_cuda_graph else tick_nonca_compute
        )
    ...
    def init_layer_cuda_graphs(self):
        ...
        print(
            f"-- init_layer_cuda_graphs: {seq_len=}, {self.layers=}, {len(self.layers)=}",
            flush=True,
        )
        ...
        try:
            for layer in self.layers:
                print(
                    f"-- init_layer_cuda_graphs: layer.init_pre_attn_cuda_graph: {layer.layer_number=}, {layer.self_attention.linear_qkv.weight.shape=}",
                    flush=True,
                )
            ...
            print(
                f"-- init_layer_cuda_graphs: layer.init_post_attn_cuda_graph: {layer.layer_number=}, {layer.self_attention.linear_qkv.weight.shape=}"
            )
            ...
            for (
                layer,
                recompute_input_ln,
                recompute_pre_mlp_ln,
                recompute_mlp,
            ) in saved_recompute_flags:
                if hasattr(layer, "recompute_input_layernorm"):
        ...
        print("-- init_layer_cuda_graphs done --")
    ...
    def ping_pong_forward(
        ...
        packed_seq_params: PingPangPackedSeqParams | None = None,
        ...
        *,
        ...
        packed_seq_params_0.stream = self.comm_stream
        packed_seq_params_1.stream = self.comm_stream
        packed_seq_params_0.dispatcher_id = 0
        packed_seq_params_1.dispatcher_id = 1

        with rng_context, outer_fp8_context:
            # Forward pass.
            if self.config.recompute_granularity == "full" and self.training:
                raise NotImplementedError(
                    "Full recompute full layer not supported in ping-pong forward yet."
                )
            else:
                arg_group = {
                ...
                for l_no in range(len(self.layers)):
                    ...
                    with self.offload_context, inner_fp8_context:
                        arg_group_0, arg_group_1, hidden_states, context = (
                            self.forward_layer_ping_pong(
    ...
    def forward_layer_ping_pong(
        ...
        with torch.cuda.nvtx.range(linear_name + ".0"):
            arg_group_0 = self.tick_nonca_compute(
                layer, prev_layer, arg_group_0, is_last_layer_post_attn=False
            )
        with torch.cuda.nvtx.range(f"forward[{l_no}].sync_tick.0"):
            tick_sync(
                ...
                arg_group_1,
                "signal" if l_no > 0 else "hidden_states",
                layer_info=f"L{l_no}",
                operation_info="tick_0",
            )
        ...
        with torch.cuda.nvtx.range(linear_name + ".1"):
            arg_group_1 = self.tick_nonca_compute(
                layer, prev_layer, arg_group_1, is_last_layer_post_attn=False
            )
        ...
        if l_no == len(self.layers) - 1:
            ...
            with torch.cuda.nvtx.range(f"forward[{l_no}].post_core_attn.0"):
                arg_group_0 = self.tick_nonca_compute(
                    layer, None, arg_group_0, is_last_layer_post_attn=True
                )
            with torch.cuda.nvtx.range(f"forward[{l_no}].sync_tick.4"):
                tick_sync(
                    ...
                    self.comm_stream,
                    arg_group_0,
                    "hidden_states",  # place holder
                    arg_group_1,
                    "signal",  # comm out
                    layer_info=f"L{l_no}",
                    operation_info="tick_4",
                )
            with torch.cuda.nvtx.range(f"forward[{l_no}].post_core_attn.1"):
                arg_group_1 = self.tick_nonca_compute(
                    layer, None, arg_group_1, is_last_layer_post_attn=True
                )
            # gathering the result
            with torch.cuda.nvtx.range(f"forward[{l_no}].gather_ping_pong"):
                hidden_states = gather_tensor(
                    [arg_group_0["hidden_states"], arg_group_1["hidden_states"]], 2
                )
    ...
    def forward(self, *args, **kwargs):
        ...
        if self._ping_pong_debug:
            assert self._debug_forward_impl in ["orig", "single_sided", "orig_reimpl"], (
                self._debug_forward_impl
            )
            if self._debug_forward_impl == "single_sided":
                if not _printed_single_sided:
                    print("Using single-sided forward implementation for debugging")
                ...
                if not _printed_orig_reimpl:
                    print("Using original reimplementation for debugging")
        ...
        if not _printed_ping_pong:
            print("Using ping-pong forward implementation")
...
def add_ping_pong_forward(block: MegatronTransformerBlock) -> PingPongTransformerBlockInterface:
    ...
    print("Patching various ping-pong forward to transformer block")

    block._normal_forward = block.forward
    block.init_value = types.MethodType(PingPongTransformerBlockInterface.init_value, block)
    ...
    block.forward = types.MethodType(PingPongTransformerBlockInterface.forward, block)
...
class PingPongGPTModel(GPTModel):
    ...
    def dummy_backward(self, packed_seq_params: PingPangPackedSeqParams):
        ...
        with torch.cuda.nvtx.range("backward(with_dummy)"):
