...
class TransformerLayer(MegatronTransformerLayer):
        ...
        if rotary_pos_emb is not None and not self.config.flash_decode:
            ...
            if q_pos_emb is not None:
                if shard_logical_range is not None:
                    with torch.cuda.nvtx.range(f"RoPE.L{self.layer_number}.query_distca"):
                        final_indices = None
                        if isinstance(packed_seq_params, MLPLayoutPackedSeqParams):
            ...
            if k_pos_emb is not None:
                if shard_logical_range is not None:
                    with torch.cuda.nvtx.range(f"RoPE.L{self.layer_number}.key_distca"):
                        final_indices = None
                        if isinstance(packed_seq_params, MLPLayoutPackedSeqParams):
                            final_indices = packed_seq_params.rope_final_indices
    ...
    def _forward_core_attn(
        ...
        # ==================================
        should_distca_sync_time_core_attn = os.getenv("D2_SYNC_TIME_CORE_ATTN", "0") == "1"
