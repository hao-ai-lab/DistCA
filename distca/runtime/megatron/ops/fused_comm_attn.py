    ...
    def __post_init__(self):
        ...
        if self.deterministic:
            ...
            if not _deterministic_warning_printed:
                print(
                    "⚠️⚠️⚠️ Using deterministic = True! This is not recommended when profiling for performance as it will degrade backward pass performance!"
                )
                _deterministic_warning_printed = True


def _qkv_to_attn_out_fwd(
    q: Tensor,
    k: Tensor,
    v: Tensor,
    fa_params: PackedSeqParams,
    fa_args: FlashAttnArgs,
):
    ...
    if False:
        ...
        print(
            "🟡 _qkv_to_attn_out_fwd: cu_seqlens_q.shape = ",
            fa_params.cu_seqlens_q,
            fa_params.cu_seqlens_q.shape,
        )
        print(
            "🟡 _qkv_to_attn_out_fwd: cu_seqlens_kv.shape = ",
            fa_params.cu_seqlens_kv,
            fa_params.cu_seqlens_kv.shape,
        )
        ...
        print(
            "🟡 _qkv_to_attn_out_fwd: alibi_slopes.shape = ",
            fa_args.alibi_slopes.shape if fa_args.alibi_slopes is not None else None,
        )
        print("🟡 _qkv_to_attn_out_fwd: return_attn_probs = ", fa_args.return_attn_probs)
        print(
            "🟡 _qkv_to_attn_out_fwd: block_table.shape = ",
            fa_args.block_table.shape if fa_args.block_table is not None else None,
        )
...
def _qkv_to_attn_out_bwd(
    ...
    dout = attn_out_grad.reshape(attn_out_grad.shape[0], num_heads_q, head_dim)
    ...
    if False:
        ...
        print(
            "🟡 _qkv_to_attn_out_bwd: alibi_slopes.shape = ",
            fa_args.alibi_slopes.shape if fa_args.alibi_slopes is not None else None,
        )
    ...
    flash_attn.flash_attn_interface._wrapped_flash_attn_varlen_backward(
        ...
        softmax_scale,
        fa_args.causal,
        fa_args.window_size[0],
        fa_args.window_size[1],
        fa_args.softcap,
...
class FusedCommAttn(torch.autograd.Function):
    ...
    def forward(
        ...
        post_a2a_qkv(
            ...
            recv_v,
            None,
            fwd_qkv_metadata.seq_lens[0].recv_seqlens,
            fwd_qkv_metadata.seq_lens[1].recv_seqlens,
        ...
        saved_tensors.extend(
            [
                bwd_fa_params.cu_seqlens_q,
                bwd_fa_params.cu_seqlens_kv,
            ]
        )
        ...
        attn_out = pre_a2a_attn_out_with_lse(
            attn_out,
            softmax_lse,
            fwd_attn_out_metadata.seq_lens[0].send_seqlens,
    ...
    def backward(ctx, signal_grad: Tensor):
        ...
        (
            bwd_qkv_grad_send_seqlens_q,
            bwd_qkv_grad_send_seqlens_k,
            bwd_q_grad_send_offset,
            bwd_k_grad_send_offset,
            bwd_v_grad_send_offset,
            bwd_attn_cu_seqlens_q,
            bwd_attn_cu_seqlens_kv,
            bwd_attn_out_qkv_recv_seqlens_q,
            bwd_attn_out_qkv_recv_seqlens_k,
            bwd_attn_out_qkv_recv_q_offset,
            bwd_attn_out_qkv_recv_k_offset,
        ...
        recv_k = torch.empty(recv_k_shape, dtype=signal_grad.dtype, device=signal_grad.device)
        recv_v = torch.empty_like(recv_k)
        ...
        dq, dk, dv = _qkv_to_attn_out_bwd(
            ...
            bwd_attn_cu_seqlens_q,
            bwd_attn_cu_seqlens_kv,
            ctx.bwd_attn_max_seqlen_q,
            ctx.bwd_attn_max_seqlen_kv,
        )
        torch.cuda.nvtx.range_pop()
...
class post_a2a_attn_out_with_lse(torch.autograd.Function):
    ...
    def forward(
        ...
        recv_attn_out = torch.empty(recv_shape, dtype=signal.dtype, device=signal.device).view(
            torch.uint8
        )
...
def dummy_backward_single_sided(
    ...
    dq, dk, dv = _qkv_to_attn_out_bwd(
        ...
        bwd_fa_params.cu_seqlens_q,
        bwd_fa_params.cu_seqlens_kv,
        bwd_fa_params.max_seqlen_q,
        bwd_fa_params.max_seqlen_kv,
    )
    torch.cuda.nvtx.range_pop()
...
def dummy_backward_send_qkv(
    packed_seq_params: PingPangSingleStepPackedSeqParams, event: torch.cuda.Event
):
    dispatcher_id = packed_seq_params.dispatcher_id
    assert dispatcher_id is not None
...
def dummy_backward(
    ...
    with torch.cuda.nvtx.range("dummy_bwd_ping_pong_1"):
        compute_done_1 = dummy_backward_single_sided(
            config,
            packed_seq_params.seq_params[1],
            dtype,
            device,
    ...
    with torch.cuda.nvtx.range("dummy_bwd_ping_pong_0"):
        compute_done_0 = dummy_backward_single_sided(
            config,
            packed_seq_params.seq_params[0],
            dtype,
            device,
            qkv_grad_all2all=False,
        )
