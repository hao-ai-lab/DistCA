...
def precompute_rope_final_indices(
    cu_seqlens: Tensor, shard_logical_range: Tensor, device: str = "cpu"
) -> Tensor:
    ...
    final_indices = torch.arange(total_tokens, device=device, dtype=torch.long) + token_offsets.to(
        device=device, dtype=torch.long
    )
    return final_indices


def apply_rotary_pos_emb_distca(
    t: Tensor,
    freqs: Tensor,
    config: TransformerConfig,
    cu_seqlens: Tensor | None = None,
    shard_logical_range: Tensor | None = None,
    final_indices: Tensor | None = None,  # Precomputed indices to avoid repeated computation
    mscale: float = 1.0,
) -> Tensor:
    """
    ...
    """
    with torch.cuda.nvtx.range("RoPE_distca.apply_rotary_pos_emb_distca_opt"):
        ...
        if final_indices is not None:
            ...
            print("[RoPE] Computing final_indices from cu_seqlens and shard_logical_range")

        if final_indices is None:
            if cu_seqlens is None or shard_logical_range is None:
                raise ValueError(
                    "Either final_indices or both cu_seqlens and shard_logical_range must be provided"
                )
            seqlens = cu_seqlens[1:] - cu_seqlens[:-1]
            offsets = shard_logical_range[:, 0] - cu_seqlens[:-1]
            token_offsets = torch.repeat_interleave(offsets, seqlens)
            final_indices = torch.arange(
                t.shape[0], device=t.device, dtype=torch.long
            ) + token_offsets.to(device=t.device, dtype=torch.long)
        else:
        ...
        with torch.cuda.nvtx.range("RoPE_distca.apply_rotary_pos_emb_bshd"):
...
def apply_rotary_pos_emb_distca_triton(
    t: Tensor,
    freqs: Tensor,
    config: TransformerConfig,
    cu_seqlens: Tensor | None = None,
    shard_logical_range: Tensor | None = None,
    final_indices: Tensor | None = None,
    mscale: float = 1.0,
) -> Tensor:
