...
from distca.runtime.metadata import (
    AlltoAllMetadata,
    DedupGradSumMetadata,
    LogicalShape,
    SeqLens,
    _get_my_rank_from_metadata,
    compute_reverse_a2a_layout_metadata,
)
from distca.runtime.shard_info import ShardInfo
from distca.runtime.utils import exclusive_cumsum, prepend_zero_fn, size_pad_by_int4
...
class _ShardCommInfo:
    ...
    replica_id: int | None  # None for q, replica id for k/v
...
def get_per_token_bytes(
    ...
    assert not (is_send_lse_in_fwd and is_resend_qkv_in_bwd), (
        "cannot be both fwd(send lse) and bwd(resend qkv)"
    )
    # compute per token hidden size
    q_bytes = element_size * (
        # if resend qkv in bwd, will send q, attn_out, attn_out_grad, lse
        hidden_size_q * 3 + lse_size_in_hidden_dtype if is_resend_qkv_in_bwd else hidden_size_q
    )
    ...
    attn_out_bytes = element_size * (
        ...
        hidden_size_q + lse_size_in_hidden_dtype if is_send_lse_in_fwd else hidden_size_q
    )
...
def _get_logical_shape(
    ...
    kv_max_cp: list[int] | None = None,
):
    """Get the logical shape of the tensor on each rank."""
    assert num_token_per_rank.shape == (world_size,)
    num_total_tokens: list[int] = [num_token_per_rank[rank].item() for rank in range(world_size)]
    return [
...
def _get_input_output_shape(
    ...
    for linear_rank in range(world_size):
        for attn_rank in range(world_size):
            q_comm_tokens = _total_tokens(linear_to_attn_q[linear_rank][attn_rank], doc_info)
    ...
    k_send_shape = _get_logical_shape(
        rank_send_tokens, world_size, k_hidden, is_kv_linear=True, kv_max_cp=max_num_dst_on_rank
    )
    ...
    for idx, comm_info in enumerate(comm_infos):
        ...
        if is_kv_linear:
            ...
            if not is_main_copy:
                ...
                recv_offset[attn_rank][id_on_attn_rank] = recv_offset[attn_rank][
                    main_copy.id_on_attn_rank
                ]
                continue
...
def _from_planner_output(
    ...
    scheduler_output = [
        sorted(doc_so, key=lambda shard: shard.logical_sid) for doc_so in scheduler_output
    ]
    ...
    linear_to_attn_qkv_bytes = pair_comm_tokens_q * q_bytes + pair_comm_tokens_k * k_bytes * 2
    # fast a2a metadata
    linear_to_attn_qkv_fa2a_metadata = get_fa2a_metadata(linear_to_attn_qkv_bytes)

    # qkv linear to attn metadata
    # init
    n_shards_send = [len(linear_shards_on_rank[l_rank]) for l_rank in range(world_size)]
    q_offset_sends = [
        torch.zeros((n_shards_send[l_rank],), dtype=torch.int64) for l_rank in range(world_size)
    ]
    k_offset_sends = [
    ...
    for l_rank in range(world_size):
        ...
        for a_rank in range(world_size):
            ...
            cur_offset_send, cur_offset_recv = _assign_offsets(
                ...
                l_rank,
                a_rank,
                linear_to_attn_q[l_rank][a_rank],
            )
            cur_offset_send, cur_offset_recv = _assign_offsets(
                cur_offset_send,
                cur_offset_recv,
                ...
                l_rank,
                a_rank,
                linear_to_attn_k[l_rank][a_rank],
                is_kv_linear=True,
            )
            cur_offset_send, cur_offset_recv = _assign_offsets(
                cur_offset_send,
    ...
    # seqlens
    (linear_to_attn_seqlens_q, linear_to_attn_seqlens_k) = _get_seqlens(
        scheduler_output,
    ...
    ## Flash Attn metadata
    attn_q_seqlens_on_rank = [torch.tensor(v, dtype=torch.int32) for v in attn_q_seqlens_on_rank]
    attn_k_seqlens_on_rank = [torch.tensor(v, dtype=torch.int32) for v in attn_k_seqlens_on_rank]
    attn_metadata = get_attn_metadata(attn_q_seqlens_on_rank, attn_k_seqlens_on_rank)

    ## Bwd metadata
    qkv_grad_attn_to_linear = compute_reverse_a2a_layout_metadata(qkv_linear_to_attn)
    dedup_metadata = []
    for a_rank in range(world_size):
        attn_k_shard_lens = linear_to_attn_seqlens_k.recv_seqlens[a_rank]
        rank_dedup = _get_dedup_grad_sum_metadata(
            attn_k_shard_lens, [linear_to_attn_k[l_rank][a_rank] for l_rank in range(world_size)]
        )
    ...
    if compute_attn_out_metadata:
        # fa2a metadata
        out_grad_linear_to_attn_bytes = pair_comm_tokens_q * attn_out_bytes
        linear_to_attn_out_grad_fa2a_metadata = get_fa2a_metadata(out_grad_linear_to_attn_bytes)
        ...
        for l_rank in range(world_size):
            for a_rank in range(world_size):
                ...
                cur_offset_send, cur_offset_recv = _assign_offsets(
                    cur_offset_send,
                    cur_offset_recv,
                    out_grad_offset_sends,
                    out_grad_offset_recvs,
                    scheduler_output,
                    attn_out_bytes,
                    l_rank,
                    a_rank,
        ...
        out_grad_send_shape = _get_logical_shape(
            pair_comm_tokens_out_grad.sum(dim=1),
            world_size,
            out_hidden,
        )
        out_grad_recv_shape = _get_logical_shape(
            pair_comm_tokens_out_grad.sum(dim=0),
            world_size,
            out_hidden,
        )
        ...
        out_attn_to_linear = compute_reverse_a2a_layout_metadata(out_grad_linear_to_attn)
        return (
            qkv_linear_to_attn,
            qkv_grad_attn_to_linear,
            out_attn_to_linear,
...
def from_planner_output(
    ...
    if is_resend_qkv:
        assert not is_pipeline_tick, (
            "is_resend_qkv is for Non-PP but resend QKV to make memory balanced"
        )
    ...
    q_bytes, k_bytes, out_bytes = get_per_token_bytes(
        hidden_size_q,
        hidden_size_kv,
        lse_size_in_hidden_dtype,
        element_size,
        is_resend_qkv_in_bwd=False,
        is_send_lse_in_fwd=is_send_lse_in_fwd,
    )
    (
        qkv_linear_to_attn,
        qkv_grad_attn_to_linear,
        ...
        out_grad_linear_to_attn,
        attn_metadata,
    ) = _from_planner_output(
        world_size,
        scheduler_output,
        q_bytes,
        k_bytes,
        element_size,
        compute_attn_out_metadata=True,
        attn_out_bytes=out_bytes,
    )
    ...
    if os.getenv("D2_DEBUG_PRINT_METADATA", "0") == "1":
        ...
        if rank % 8 == 1:
            rich.print("from_planner_output: qkv_linear_to_attn=", qkv_linear_to_attn)
            rich.print("from_planner_output: qkv_grad_attn_to_linear=", qkv_grad_attn_to_linear)
            rich.print("from_planner_output: out_attn_to_linear=", out_attn_to_linear)
            rich.print("from_planner_output: out_grad_linear_to_attn=", out_grad_linear_to_attn)
            rich.print("from_planner_output: attn_metadata=", attn_metadata)
...
def backward_from_planner_output(
    world_size: int,
    scheduler_output_bwd: Sequence[Sequence[ShardInfo]] | None,
    hidden_size_q: int,
    hidden_size_kv: int,
    lse_size_in_hidden_dtype: int,
    element_size: int,
    ...
    q_bytes, k_bytes, _ = get_per_token_bytes(
        ...
        lse_size_in_hidden_dtype,
        element_size,
        is_resend_qkv_in_bwd=True,
        is_send_lse_in_fwd=False,
    )
    qkv_resend_and_out_grad_linear_to_attn, _, bwd_attn_metadata = _from_planner_output(
        ...
        k_bytes,
        element_size,
        compute_attn_out_metadata=False,
        attn_out_bytes=None,
    )
    ...
    if os.getenv("D2_DEBUG_PRINT_METADATA", "0") == "1":
        ...
        if rank % 8 == 1:
            rich.print(
                "backward_from_planner_output: qkv_resend_and_out_grad_linear_to_attn=",
                qkv_resend_and_out_grad_linear_to_attn.__better_print__(),
            )
            rich.print(
                "backward_from_planner_output: qkv_grad_attn_to_linear=",
                qkv_grad_attn_to_linear.__better_print__(),
            )
