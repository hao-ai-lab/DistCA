...
from distca.simulator.optimizers.samples import batch_documents, sample_wlbllm_docs_upsample
...
def calculate_recommended_buffer_size(
    ...
    if verbose:
        rich.print(
            f"Max size overall: ({max_send_sz_mb_overall}, {max_recv_sz_mb_overall}, {max_send_last_offset_overall})"
        )

    recommended_buffer_size = max(
        max_send_sz_mb_overall, max_recv_sz_mb_overall, max_send_last_offset_overall
    )
