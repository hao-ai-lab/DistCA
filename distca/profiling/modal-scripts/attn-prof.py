...
vllm_image = modal.Image.debian_slim(python_version="3.12").pip_install("vllm-flash-attn")
...
def run_single_sequence():
    ...
    with open(f"compute-attn-{gpu_type}.psv", "a") as f:
        ...
        print_dual("gpu_type|op|total_len|batch_size|tp|cp|latency(ms)|hqo|hkv|d|batch")
                ...
                for tp in [1, 2, 4, 8]:
                    for cp in [1, 2, 4, 8]:
                        try:
                            ...
                            print_dual(
                                f"{gpu_type}|attn|{total_len}|{batch_size}|{tp}|{cp}|{avg_duration:.2f}|{hqo}|{hkv}|{d}|{batch}"
                            )
                        except Exception as e:
                            print(
                                f"Batch: {batch}, TP: {tp}, CP: {cp}, Model Config: {model_config} encountered error: {e}"
                            )
                            continue
...
def run_amortized_batch_sequence():
    ...
    # max_seq_len = 128*K
    batches = [f"[{s}] * ({max_seq_len // s})" for s in seq_lens]
    ...
    with open(f"compute-attn-{gpu_type}.psv", "a") as f:
        ...
        print_dual(
            "gpu_type|op|total_len|batch_size|tp|cp|latency(ms)|total_latency(ms)|hqo|hkv|d|batch"
        )
                ...
                for tp in [1, 2, 4, 8]:
                    for cp in [1, 2, 4, 8]:
                        try:
                            ...
                            print_dual(
                                f"{gpu_type}|attn|{total_len}|{batch_size}|{tp}|{cp}|{per_doc_avg_duration}|{total_avg_duration}|{hqo}|{hkv}|{d}|{batch}"
                            )
                        except Exception as e:
                            print(
                                f"Batch: {batch}, TP: {tp}, CP: {cp}, Model Config: {model_config} encountered error: {e}"
