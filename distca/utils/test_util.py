...
from distca.planner.planner import batch_to_items_with_dummy
...
from distca.runtime.shard_info import ShardInfo
...
class BaseWorker:
    ...
    def init_nvshmem(self, buffer_size: int, local_rank: int = None):
        ...
        DispatcherWrapper.init(self.rank, local_rank, self.world_size, buffer_size, uid)
...
class MegatronBaseWorker(BaseWorker):
    ...
    def init_nvshmem(
        self, buffer_size: int, parallel_config: ParallelConfig, local_rank: int = None
    ):
        if local_rank is None:
            local_rank = int(os.getenv("LOCAL_RANK"))
        ...
        DispatcherWrapper.init(as_rank, local_rank, as_world_size, buffer_size, uid)

    def init_comm(
        self, buffer_size: int, parallel_config: ParallelConfig, local_rank: int | None = None
    ):
...
def init_worker_torch_distributed(world_size, buffer_size, worker_cls, parallel_config):
    assert world_size == int(os.environ.get("WORLD_SIZE")), (
        f"world_size: {world_size} != WORLD_SIZE: {os.environ.get('WORLD_SIZE')}"
    )
...
def create_random_dispatch_from_existing(
    per_rank_shard_lens: list[list[list[int]]],
    world_size: int,
    merge_ranks: bool = True,
):
    assert len(per_rank_shard_lens) == world_size
    num_shards = sum(
        len(doc_shards) for rank_shards in per_rank_shard_lens for doc_shards in rank_shards
    )
    dsts = torch.concat(
        [
            torch.range(0, world_size - 1, dtype=torch.int32, device="cpu"),
            torch.randint(
                0, world_size, (num_shards - world_size,), dtype=torch.int32, device="cpu"
            ),
        ],
        dim=0,
    )
    ...
    for rank in range(world_size):
        ...
        for doc_shards in per_rank_shard_lens[rank]:
            ...
            for sid, shard_len in enumerate(doc_shards):
                doc_outs.append(
                    ShardInfo(
                        rid=rank, dispatch_rid=next(dsts_iter), logical_sid=sid, shard_len=shard_len
                    )
                )
    ...
    for rank in range(world_size):
        if not has_shard_src[rank]:
            scheduler_output.append(
                [ShardInfo(rid=rank, dispatch_rid=rank, logical_sid=0, shard_len=min_shard_len)]
            )
...
def random_shard_info_linear_layout_dp(
    ...
    for r in range(world_size):
        ...
        for doc_len in doc_lens:
            ...
            shards = [
                ShardInfo(
                    r,
                    dispatch_rid=random.randint(0, world_size - 1),
                    logical_sid=shard_id,
                    shard_len=shard_len,
                )
    ...
    if create_backward_redispatch:
        per_rank_shard_lens = [
            [[s.shard_len for s in ds] for ds in so] for so in scheduler_output_per_rank
        ]
...
Historically this module used `distca.utils.global_batch_provider`, which only supports
synthetic seq-lens ("wlbllm"/"prolong").

We now route planner-mode batch fetching through `distca.utils.training_utils`, which
...
def create_pipeline_doclens(
    ...
    if add_dummy:
        ...
        pp_head_new_doc_len = [
            [total_token_on_rank // dp_size] for _ in range(dp_size)
        ...
        if use_planner == False:
            ...
            debug_print(
                f"In util.py, before calling get_next_batch: GLOBAL_BATCH is: {GLOBAL_BATCH}"
            )
            num_batches = num_batches or dp_size
            pp_head_new_doc_len = get_next_batch(num_batches)
    debug_print(
        f"In util.py, after calling get_next_batch: pp_head_new_doc_len is: {pp_head_new_doc_len}"
    )
    ...
    if ref_doc_lens is not None:
        ...
        other_pp_doc_len = [
            [total_token_on_rank // dp_size] for _ in range(dummy_fwd_num)
        ]  # FIXME: using total_token_on_rank for cudagraph to work out-of-box, can be tp_size when cudagraph is disabled.
    ...
    if return_original_doclen:
        debug_print(
            "In util.py, return_original_doclen is True, returning (tick_per_rank_doc_len, pp_head_new_doc_len)"
        )
...
def create_qkv_dispatch_pipeline_tick(
    ...
    ref_doc_lens: torch.Tensor | None,
    add_dummy: bool,
    tp_size: int,
    dp_size: int,
    ...
    """
    debug_print(
        f"🟡 Inside create_qkv_dispatch_pipeline_tick: {world_size = }. Please check the world size as this is acutlaly should just be dp * pp."
    )
    ...
    debug_print(
        f"🟡 cur_tick_per_rank_doc_lens: {cur_tick_per_rank_doc_lens}, len(cur_tick_per_rank_doc_lens): {len(cur_tick_per_rank_doc_lens)}"
    )
    debug_print(
        f"🟡 original_cur_tick_per_rank_doc_lens: {original_cur_tick_per_rank_doc_lens}, len(original_cur_tick_per_rank_doc_lens): {len(original_cur_tick_per_rank_doc_lens)}"
    )

    # Try different tolerance_factor values to find one that passes the buffer size check
    tolerance_factors_to_try = [
        0.05,
        0.1,
        0.15,
        0.2,
    ...
    for tolerance_factor in tolerance_factors_to_try:
        ...
        if use_planner:
            ...
            pp_size = (
                world_size // dp_size
            )  # in this function, world size is actual as_world_size. pp = world_size // dp_size
        ...
        end_time = time.time()
        debug_print(
            f"🟡 🟡 create_qkv_dispatch_pipeline_tick - fwd_planner_out duration: {(end_time - start_time):.2f} sec",
            flush=True,
        )
        ...
        if len(cur_tick_per_rank_doc_lens) < world_size:
            ...
            bwd_tick_per_rank_doc_lens = _block_reverse_list(
                cur_tick_per_rank_doc_lens, num_batches
            )
        else:
            # None CP flip logic.
        ...
        end_time = time.time()
        debug_print(
            f"🟡 🟡 create_qkv_dispatch_pipeline_tick - cp flip duration: {(end_time - start_time):.2f} sec"
        )

        # FIXME: Properly pass the output dir down here.
        start_time = time.time()
        from distca.utils.network_inspect import inspect_network_metadata
        ...
        end_time = time.time()
        debug_print(
            f"🟡 🟡 create_qkv_dispatch_pipeline_tick - network_inspect duration: {(end_time - start_time):.2f} sec"
        )
        ...
        def is_buffer_size_enough(buffer_size, network_buffer_requirements):
            ...
            debug_print(
                f"🟡 Got max_buffer_budget_all_rank = {max_buffer_budget_all_rank / 1024**3} GB ({max_buffer_budget_all_rank}), buffer_size = {buffer_size / 1024**3} GB ({buffer_size}). Is enough: {(buffer_size >= max_buffer_budget_all_rank) = }"
            )
        ...
        if is_buffer_size_enough(buffer_size, network_buffer_requirements):
            ...
            debug_print(
                f"🟡 tolerance_factor {tolerance_factor} failed: buffer_size={buffer_size}, required={network_buffer_requirements['max_buffer_budget_all_rank']}"
            )

    # If we reach here, none of the tolerance_factors worked
    raise ValueError(
        f"None of the tolerance factors {tolerance_factors_to_try} resulted in sufficient buffer size. "
...
def gen_seq_lens(world_size: int, num_seqs: int, total_len: int) -> torch.Tensor:
    ratio = (
        torch.rand((world_size, num_seqs)) + 0.25 / num_seqs
    )  # Use a min value to guarantee that the sequence is not too short (0 after rounding)
