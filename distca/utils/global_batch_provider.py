...
def balance_ping_pong_with_mb(
    ...
    assert len(seq_lens) == (mb * batch_size * 2), (
        f"seq_lens should be divisible by {mb * batch_size * 2}, but got {len(seq_lens)}"
    )
...
def get_next_batch(dp_size):
    ...
    if GLOBAL_BATCH is None:
        raise RuntimeError(
            "GLOBAL_BATCH has not been initialized. Call setup_global_batch() first."
        )
