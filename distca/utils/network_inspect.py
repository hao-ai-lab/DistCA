...
def inspect_network_metadata(
    metadata: "FastAllToAllMetadata_Tuple",
    is_ping,
    sample_id,
    tolerance_factor,
    output_dir,
    ...
    qkv_fwd_metadata__send_transfer_sz_to_others = exclude_self_and_sum(
        qkv_fwd_metadata__send_transfer_sz
    )
    qkv_fwd_metadata__recv_transfer_sz_to_others = exclude_self_and_sum(
        qkv_fwd_metadata__recv_transfer_sz
    )
    ...
    attn_out_fwd_metadata__send_transfer_sz_to_others = exclude_self_and_sum(
        attn_out_fwd_metadata__send_transfer_sz
    )
    attn_out_fwd_metadata__recv_transfer_sz_to_others = exclude_self_and_sum(
        attn_out_fwd_metadata__recv_transfer_sz
    )
                    ...
                    json.dumps(
                        {
                            ...
                            "qkv_fwd_metadata__send_transfer_sz_mb": (
                                qkv_fwd_metadata__send_transfer_sz // (1024 * 1024)
                            ).tolist(),
                            "qkv_fwd_metadata__recv_transfer_sz_mb": (
                                qkv_fwd_metadata__recv_transfer_sz // (1024 * 1024)
                            ).tolist(),
                            "attn_out_fwd_metadata__send_transfer_sz_mb": (
                                attn_out_fwd_metadata__send_transfer_sz // (1024 * 1024)
                            ).tolist(),
                            "attn_out_fwd_metadata__recv_transfer_sz_mb": (
                                attn_out_fwd_metadata__recv_transfer_sz // (1024 * 1024)
                            ).tolist(),
                            "qkv_fwd_metadata__send_transfer_sz_mb_to_others": (
                                qkv_fwd_metadata__send_transfer_sz_to_others // (1024 * 1024)
                            ).tolist(),
                            "qkv_fwd_metadata__recv_transfer_sz_mb_from_others": (
                                qkv_fwd_metadata__recv_transfer_sz_to_others // (1024 * 1024)
                            ).tolist(),
                            "max_comm_budget_all_rank_mb": max_comm_budget_all_rank
                            // (1024 * 1024),
                            "max_buffer_budget_all_rank_mb": max_buffer_budget_all_rank
                            // (1024 * 1024),
