...
from distca.simulator.optimizers.attnserver import AttnServerSolver
from distca.simulator.optimizers.wlbllm import WlbLlmSolver
...
def run_comparison(
    ...
    for i, batch in enumerate(batches):
        ...
        (speedup, wlbllm_best_solution, attnserver_solution, wlb_time, attn_time) = (
        ...
        print(
            f"Iteration {i + 1}: WLB-LLM Time: {wlb_time:.2f}s, AttnServer Time: {attn_time:.2f}s"
        )
        ...
        def plot_speedup_distribution():
            ...
            for count, x in zip(n, bins[:-1]):
                plt.text(
                    x + (bins[1] - bins[0]) / 2, count, str(int(count)), ha="center", va="bottom"
                )
