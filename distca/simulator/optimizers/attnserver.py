...
def print_latency_table(
    batch: list[int], parallel_plan: list[tuple[int, int]], latency: np.ndarray
):
    ...
    def print_solution(self):
        ...
        for i, (worker, plan) in enumerate(self.worker2plan.items()):
            if len(self.batches[i]) == 0:
                continue
            print(
                f"- Worker {i}: {plan} docs {self.batches[i]} - latency: {self.lat_worker[i] / 1000} ms"
            )
...
class AttnServerSolver:
    ...
    def get_latency_table(
        self, batch: list[int], parallel_plan: list[tuple[int, int]], num_total_devices: int
    ):
        hqo = 64
        hkv = 4
        d = 128
        ...
        for j, (tp, cp) in enumerate(parallel_plan):
            ...
            for k in range(len(batch)):
                ...
                if tp * cp == 0:
                    ...
                    print(
                        f"[AttnServer] [{tp=}, {cp=}] d: {doc_length}, latency: {(lat * 1000):.1f} us, attn_time: {attn_time * 1000:.2f} us"
                    )
    ...
    def get_min_mlp_time(self, batch, total_num_devices):
        ...
        print(
            f"[AttnServer] MLP candidates: {(min_mlp_time * 1000):.2f} us (tp={min_mlp_time_tp}, cp={min_mlp_time_cp})"
        )
    ...
    def solve(
        ...
        y = {
            (k, i): model.NewBoolVar(f"y_{k}_{i}")
        ...
        # 4. Resource budget
        total_devices = sum(
            resource[j] * x[i, j] for i in range(num_workers) for j in range(num_plans)
        )
            ...
            for j in range(num_plans):
                if solver.Value(x[i, j]) == 1:
                    ...
                    if xs[i] != (0, 0):
                        ...
                        print(
                            f"Worker {i} plan {j} = {parallel_plan[j]} latency {(worker_lat):.2f} us"
                        )
