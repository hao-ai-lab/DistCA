...
export MODEL_PATH_GLOBAL=${MODEL_PATH_GLOBAL:-codellama/CodeLlama-34b-hf}
export MODEL_PATH_NORMALIZED_GLOBAL=${MODEL_PATH_NORMALIZED_GLOBAL:-$(echo "$MODEL_PATH_GLOBAL" | sed 's/\//_/g')}

model_config="${MODEL_PATH_GLOBAL} 131072 48"
param_configs_cases=()
...
while IFS= read -r line; do
    ...
    if [[ "$SKIP_PARTIAL" == "1" ]] && [[ "$LIST_RUNS_FULL" != "1" ]]; then
        ...
        if read -r nnodes batch_size microbatch_size num_tokens mode cp_size pp_size tp_size comment env_var <<< "$line"; then
            # Create model_path_normalized using the global model path
            model_path_normalized="$MODEL_PATH_NORMALIZED_GLOBAL"
...
for case in "${cases[@]}"; do
    ...
    # Create model_path_normalized for folder search (fall back to global)
    model_path_normalized=$(echo "${model_path:-$MODEL_PATH_GLOBAL}" | sed 's/\//_/g')
...
for config in "${cases[@]}"; do
    ...
    export MODEL_PATH=${model_path:-$MODEL_PATH_GLOBAL}
