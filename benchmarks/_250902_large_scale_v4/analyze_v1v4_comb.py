...
DF_INCLUDE_BENCHMARK_SAMPLE = False
...
for exp_name in tqdm(a):
    ...
    if run_successfully and DF_INCLUDE_BENCHMARK:
        ...
        for sample in samples:
            ...
            if DF_INCLUDE_BENCHMARK_SAMPLE:
                sample_datas[f"sample_data[{sample_id}]"] = sample
                pass

    # Read the runtime
    # Experiment runtime: 260 seconds
    slurm_log_file = f"{log_path}/{exp_name}/slurm.stdout"
    experiment_runtime = None
    with open(slurm_log_file, "r") as f:
        log_text = f.read()
        if "Experiment runtime:" in log_text:
            try:
                # Just get the string. No need to parse the number.
                experiment_runtime = log_text.split("Experiment runtime:")[1].split("seconds")[0].strip()
                benchmark_datas['experiment_runtime'] = experiment_runtime
                pass
    ...
    results.append(dict(
        ...
        slurm_runtime=experiment_runtime,
...
for gid, group in df_display.groupby('gid'):
    ...
    for idx, d2_row in d2_rows.iterrows():
        ...
        if not is_wlbllm_all_success:
            ...
            speedup += f" (🟡)"
        exp_name = d2_row['exp_name']
        speedups.append((exp_name, speedup))

# Add speedup column
df_display['speedup'] = ""
for exp_name, speedup in speedups:
    df_display.loc[df_display['exp_name'] == exp_name, 'speedup'] = speedup
...
for gid, group in df_display.groupby('gid'):
    ...
    # success
    is_wlbllm_all_success = wlbllm_rows['success'].all().item()
