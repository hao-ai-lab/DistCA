...
# Implement a formatted item to metadata function 
from d2.runtime.shard_info import items_into_shardinfos, plan_to_metadata
def test_balanced_flops_formatted(
    world_size_, total_seq_len_, seq_lens, max_cp_degree_, 
    verbose=False, return_intermediate=False, return_mlp_no_shard_seq_lens=False,
):
    from d2.planner.equal_flops import (
    ...
    rank = torch.distributed.get_rank()
    if rank == 0:
        rich.print("Enter PB's Formated metadata computation")
        rich.print(f"Generate Sample ID={ITERATION_ID}: {items_list}")
    ...
    if rank == 0:
        rich.print("items_post_postprocessed =", items)
    # items => List[List[ShardInfo]]
    shardinfos = items_into_shardinfos(items)
    # List[List[ShardInfo]] => All Metadata needed
    ret = plan_to_metadata(
        world_size_, shardinfos, return_intermediate=return_intermediate,
...
def create_one_batch_balanced_flops(
    ...
    ) = test_balanced_flops_formatted(
        world_size, total_seq_len, num_seqs, max_cp_degree,
        return_intermediate=True, return_mlp_no_shard_seq_lens=True
    )

    (
        fwd_q_metadata_old, rev_q_metadata_old, fwd_k_metadata_old, rev_k_metadata_old,
        attention_metadata_attn_layout_old, intermediates_old, seq_lens_old
    ) = test_create_qkv_dispatch_balanced_flops(
        world_size, total_seq_len, num_seqs, max_cp_degree,
        return_intermediate=True, return_mlp_no_shard_seq_lens=True
    )
    # Compare new metadata is totally same with all old metadata
    # Compare each metadata object from the new and old implementations
    rank = torch.distributed.get_rank()
    if rank == 0:
        rich.print("fwd_q_metadata =", fwd_q_metadata)
        rich.print("fwd_q_metadata_old =", fwd_q_metadata_old)
        rich.print("rev_q_metadata =", rev_q_metadata)
        rich.print("rev_q_metadata_old =", rev_q_metadata_old)
        rich.print("fwd_k_metadata =", fwd_k_metadata)
        rich.print("fwd_k_metadata_old =", fwd_k_metadata_old)
        rich.print("rev_k_metadata =", rev_k_metadata)
        rich.print("rev_k_metadata_old =", rev_k_metadata_old)
        rich.print("attention_metadata_attn_layout =", attention_metadata_attn_layout)
        rich.print("attention_metadata_attn_layout_old =", attention_metadata_attn_layout_old)
        rich.print("intermediates =", intermediates)
        rich.print("intermediates_old =", intermediates_old)
        rich.print("seq_lens =", seq_lens)
        rich.print("seq_lens_old =", seq_lens_old)
