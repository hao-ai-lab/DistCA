...
from test_fa2a_metadata import (
    simulate_fa2a,
    simulate_fa2a_copy_cp,
    simulate_fa2a_copy_non_cp,
)
from test_util import create_random_shard_info

from distca.runtime.attn_kernels.ops import a2a_memcpy_cp, a2a_memcpy_non_cp
...
def test(args):
    ...
    scheduler_output, _ = create_random_shard_info(
        args.seed, world_size, num_doc, max_cp_degree, min_shard_len=8, tot_num_token=total_seq_len
    )
    ...
    tensor_q = (
        torch.rand((world_size, total_seq_len, hidden_size_q), device=device, dtype=dtype) + 1
    )  # + 1 to avoid zeros
    tensor_k = (
        torch.rand((world_size, total_seq_len, hidden_size_k), device=device, dtype=dtype) + 1
    )
    tensor_v = (
        torch.rand((world_size, total_seq_len, hidden_size_k), device=device, dtype=dtype) + 1
    )
    ...
    src_buffer = torch.zeros((world_size, max_send_bytes), dtype=dtype, device=device)
    dst_buffer = torch.zeros((world_size, max_recv_bytes), dtype=dtype, device=device)
    ...
    for rank in range(world_size):
        ...
        a2a_memcpy_non_cp(
            q_shard.cuda(),
            q_src_offsets.long().cuda(),
            q_src_seqlen.long().cuda(),
            to_nvshmem=True,
            buffer=src_shard_out.cuda(),
        )
        ...
        a2a_memcpy_cp(
            k_shard.cuda(),
            do_shard,
            k_src_offsets.long().cuda(),
            kv_src_seqlen.long().cuda(),
            to_nvshmem=True,
    ...
    for rank in range(world_size):
        ...
        a2a_memcpy_non_cp(
            dst_q_shard_out,
            q_recv_offsets.long().cuda(),
            recv_seqlen_q.long().cuda(),
            to_nvshmem=False,
            buffer=dst_shard.cuda(),
        )
        torch.cuda.synchronize()
        ...
        a2a_memcpy_non_cp(
            dst_k_shard_out,
            k_recv_offsets.long().cuda(),
            recv_seqlen_kv.long().cuda(),
            to_nvshmem=False,
            buffer=dst_shard.cuda(),
        )
        torch.cuda.synchronize()
    ...
    for rank in range(world_size):
        ...
        a2a_memcpy_non_cp(
            attn_q_shard,
            q_send_offsets,
            metadata_slice.seq_lens[0].send_seqlens,
            to_nvshmem=True,
            buffer=dst_shard,
        )
        ...
        a2a_memcpy_non_cp(
            attn_k_shard,
            k_send_offsets,
            metadata_slice.seq_lens[1].send_seqlens,
            to_nvshmem=True,
            buffer=dst_shard,
            copy_seq_mask=copy_seq_mask,
        )
        a2a_memcpy_non_cp(
            attn_v_shard,
            v_send_offsets,
    ...
    grad_k = torch.zeros(
        (world_size, max_cp_degree, total_seq_len, hidden_size_k), device=device, dtype=dtype
    )
    grad_v = grad_k.clone()
    for rank in range(world_size):
        metadata_slice = qkv_bwd_fa2a_metadata.get_slice(rank).normalize()
        grad_q_recv_offsets, grad_k_recv_offsets, grad_v_recv_offsets = (
            metadata_slice.recv_memcpy_metadata
        )
        ...
        torch.testing.assert_close(
            grad_k_shard_cor.sum(dim=0) / (grad_k_shard_cor > 0).sum(dim=0), tensor_k[rank]
        )
        grad_k_shard_out = grad_k_shard.clone()
        a2a_memcpy_cp(
            grad_k_shard_out.cuda(),
            grad_do_shard,
