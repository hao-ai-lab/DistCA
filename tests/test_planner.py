...
def verification_layout(
    originals: list[dict[str, Any]], replans: list[dict[str, Any]], verbose: bool = False
) -> bool:
    """
    ...
    for seqid, items in grouped_replans.items():
        ...
        if original_q is None:
            console.print(
                f"[bold red][FAIL][/bold red] Cannot find original item(s) for (seqid={seqid})"
            )
            overall_result = False
            continue

        rlog(
            f"Original total 'q' (summed across all ranks): [bold yellow]{original_q}[/bold yellow]"
        )
        ...
        rlog(
            f"Sum of 'q' in all replanned parts for this sequence: [bold yellow]{replan_q_sum}[/bold yellow]"
        )
        ...
        if replan_q_sum == original_q:
            ...
            console.print(
                f"[bold red][FAIL][/bold red] 'q' sum mismatch for seqid={seqid}! Expected: {original_q}, Got: {replan_q_sum}"
            )
            overall_result = False
            continue
        ...
        if len(sorted_offloaded) > 1:
            for i in range(1, len(sorted_offloaded)):
                ...
                if kv_diff != q_curr:
                    console.print(
                        f"    [bold red][FAIL][/bold red] Rule violated for seqid={seqid}: kv_diff ({kv_diff}) != q ({q_curr})"
                    )
        ...
        if kv_check_passed:
            rlog(
                "[bold green][PASS][/bold green] 'kv' difference rule holds for all adjacent pairs."
            )
...
def run_flops_balance_test(
    ...
    if conservation_passed:
        ...
        console.print(
            f"[bold red][FAIL][/bold red] Total FLOPs do not match! Original: {total_original_flops:,.2f}, Replanned: {total_replanned_flops:,.2f}"
        )
    ...
    if world_size > 0:
        ...
        rlog(
            f"Acceptable Range:      [bold blue][{lower_bound:,.2f}, {upper_bound:,.2f}][/bold blue]"
        )
    ...
    if balancing_passed:
        ...
        console.print(
            "[bold red][FAIL][/bold red] At least one GPU is outside the tolerance range."
        )
...
def generate_random_rank_batches(
    num_ranks: int, total_tokens_per_rank: int, max_sequences_per_rank: int
) -> list[list[int]]:
    """
    ...
    for _ in range(num_ranks):
        ...
        sequences_for_this_rank = generate_random_split(
            total_tokens_per_rank, num_seq_for_this_rank
        )
...
def generate_random_even_numbers_with_total_sum(
    num_ranks: int, total_tokens_per_rank: int
) -> list[int]:
    """
...
def test_mlp_seq_len():
    ...
    dp_cp_test_items = batch_to_items_general(
        batches,
        num_tokens_per_rank=num_batched_token,
        DP_degree=dp_degree,
        model_config=model_config,
    )
    ...
    rich.print("[bold green][PASS][/bold green] test_mlp_seq_len Passed MLP DP test")
    ...
    rich.print("[bold green][PASS][/bold green] test_mlp_seq_len Passed MLP CP test")
    return
...
def test_batch_to_items_with_dummy():
    ...
    def compare_items(generated_items, expected_items):
        ...
        assert all(repr(g) == repr(e) for g, e in zip(generated_items, expected_items)), (
            "Item lists do not match"
        )
        rich.print(
            "[bold green][PASS][/bold green] batch_to_items_with_dummy comparison successful!"
        )
    ...
    list_items = batch_to_items_with_dummy(
        batches=batches,
    ...
    list_items_1 = batch_to_items_with_dummy(
        batches=batches,
        num_tokens_per_rank=num_tokens_per_rank,
        as_world_size=as_world_size,
        model_config=model_config,
    ...
    num_tokens_per_rank = 512
    list_items_2 = batch_to_items_with_dummy(
        batches=batches_2,
        num_tokens_per_rank=num_tokens_per_rank,
        as_world_size=as_world_size,
        model_config=model_config,
    ...
    num_tokens_per_rank = 512
    list_items_3 = batch_to_items_with_dummy(
        batches=batches_3,
        num_tokens_per_rank=num_tokens_per_rank,
        as_world_size=as_world_size,
        model_config=model_config,
...
def test_cp_list_to_mlp_list():
    ...
    result_1 = cp_list_to_mlp_list(
        cp_list_1, as_world_size=4, num_token_per_rank=num_token_per_rank
    )
    ...
    num_token_per_rank = 1024
    result_2 = cp_list_to_mlp_list(
        cp_list_2, as_world_size=4, num_token_per_rank=num_token_per_rank
    )
    ...
    result_3 = cp_list_to_mlp_list(
        cp_list_3, as_world_size=12, num_token_per_rank=num_token_per_rank
    )
    assert result_3 == [
        [256, 128, 128],
        [256, 256],
    ...
    result_4 = cp_list_to_mlp_list(
        cp_list_4, as_world_size=16, num_token_per_rank=num_token_per_rank
    )
    assert result_4 == [
        [8],
        [8],
    ...
    num_token_per_rank = 2048
    result_3 = cp_list_to_mlp_list(
        cp_list_3, as_world_size=16, num_token_per_rank=num_token_per_rank
    )
    assert result_3 == [
        [1376, 336, 336],
        [1024, 1024],
...
def test_batch_to_items_with_dummy_pp_fwd_bwd():
    ...
    reversed_items = batch_to_items_with_dummy(
        batches=reversed_batches,
        num_tokens_per_rank=num_tokens_per_rank,
        as_world_size=as_world_size,
        model_config=model_config,
    )
