...
from distca.planner.planner import Planner, batch_to_items_general
...
def mock_apply_rotary_pos_emb_bshd(
    t, freqs, rotary_interleaved, multi_latent_attention, mscale=1.0
):
    return t + freqs
...
def test_rope_with_logical_range():
    ...
    freqs = (
        ...
        .view(max_seq_len, 1, 1, 1)
        .expand(max_seq_len, 1, 1, hidden_dim)
    )
    ...
    shard_logical_range = torch.tensor([[100, 102], [0, 3]], dtype=torch.long)
    ...
    rich.print("[bold green][PASS][/bold green] Discontinuous Ranges check passed.")

    rich.print(
        "[bold green][PASS][/bold green] test_rope_with_logical_range Completed Successfully"
    )
...
def test_mlp_physical_length_and_logical_range():
    ...
    fa2a_metadata_0, as_attn_metadata_0, mlp_shard_len_0, shard_logical_range_0 = planner.plan(
        dp_cp_test_items, device="cpu"
    )
    ...
    rich.print(
        "[bold green][PASS][/bold green] test_mlp_seq_len Passed MLP CP test (Head/Tail Policy)"
    )
    return
...
def test_end_to_end_rope_verification():
    rich.print(
        "[bold yellow]🟡 Starting End-to-End RoPE Verification (Planner -> RoPE)...[/bold yellow]"
    )
    ...
    dp_cp_items = batch_to_items_general(
        batches,
        num_tokens_per_rank=num_batched_token,
        DP_degree=dp_degree,
        model_config=model_config,
    ...
    with patch(patch_path, side_effect=mock_apply_rotary_pos_emb_bshd):
        for rank_id in range(world_size):
            ...
            if not torch.allclose(actual_vals, expected_tensor):
                ...
                rich.print(
                    f"[bold red]FAIL[/bold red] Rank {rank_id} Value Mismatch at index {first_diff}"
                )
                ...
                for i, l in enumerate(phys_lens):
                    ...
                    first_diff -= l
                rich.print(
                    f"  Error occurred in Shard {shard_idx} (PhysLen: {phys_lens[shard_idx]})"
                )
    ...
    rich.print(
        "[bold green][PASS][/bold green] End-to-End RoPE Verification Completed Successfully."
    )
...
def test_end_to_end_rope_verification_triton():
    ...
    with patch(patch_path, side_effect=mock_apply_rotary_pos_emb_bshd):
        for rank_id in range(world_size):
            ...
            final_indice = precompute_rope_final_indices(
                cu_seqlens, shard_logical_range, device="cpu"
            ).to(device)
            ...
            if not torch.allclose(actual_vals, expected_values, atol=1e-5):
                diff_indices = torch.nonzero(
                    torch.abs(actual_vals - expected_values) > 1e-5
                ).flatten()
                first_diff = diff_indices[0].item()
                rich.print(
                    f"[bold red]FAIL[/bold red] Rank {rank_id} Value Mismatch at index {first_diff}"
