...
def verification_layout(originals, replans):
    ...
    for (src_gpuid, seqid), items in grouped_replans.items():
        ...
        console.print(
            f"[bold]----- Verifying Sequence (src_gpuid={src_gpuid}, seqid={seqid}) -----[/bold]"
        )

        original_item = original_map.get((src_gpuid, seqid))
        if not original_item:
            console.print(
                f"[bold red][FAIL][/bold red] Cannot find original item for (src_gpuid={src_gpuid}, seqid={seqid})"
            )
        ...
        if replan_q_sum == original_q:
            ...
            console.print(
                f"[bold red][FAIL][/bold red] 'q' sum mismatch! Expected: {original_q}, Got: {replan_q_sum}"
            )
            overall_result = False
        ...
        if len(sorted_offloaded) > 1:
            for i in range(1, len(sorted_offloaded)):
                ...
                console.print(
                    f"  - Checking item {i}: kv_diff ({kv_curr} - {kv_prev}) = [bold yellow]{kv_diff}[/bold yellow]. Comparing with current q: [bold yellow]{q_curr}[/bold yellow]"
                )
                if kv_diff != q_curr:
                    console.print(
                        f"    [bold red][FAIL][/bold red] Rule violated: kv_diff ({kv_diff}) != q ({q_curr})"
                    )
...
def test_general_split():
    ...
    item_to_split = Item(model_config, seq_len, 1, 0, 0, original_item_data, is_original=True)

    original_total_flops = item_to_split.get_flops()

    console.print("[bold magenta] Starting General Split Test [/bold magenta]")
    console.print(f"[bold blue]Original Item on GPU {item_to_split.gpuid}:[/bold blue]")
    console.print(
        f"  q={item_to_split.complete_item['q']}, kv={item_to_split.complete_item['kv']}, flops={original_total_flops:.2f}\n"
    )
    ...
    for i in range(num_splits):
        ...
        if flops_before_this_split < 2:
            console.print(
                f"[yellow]Item has only {flops_before_this_split:.2f} FLOPs left. Stopping split process.[/yellow]"
            )
            break
        ...
        console.print(
            f"[bold blue]Action:[/bold blue] Splitting {flops_to_move:.2f} FLOPs from GPU {item_to_split.gpuid} to GPU {remote_gpuid}..."
        )
        ...
        if new_item:
            ...
            console.print(
                f"[green]✅ Step {i + 1} Verification Passed:[/green] FLOPs are conserved for this split."
            )
            all_items.append(new_item)
        else:
            console.print(
                f"[yellow]Split {i + 1} did not produce a new item. Probably requested FLOPs were too small/big.[/yellow]"
            )

        if item_to_split.gpuid != item_to_split.src_gpuid:
            console.print("Entire item is moved")
            break
    ...
    console.print("[bold magenta]🏁 Final Verification 🏁[/bold magenta]")
    ...
    console.print(
        "[bold green]✅✅✅ All tests passed! Total FLOPs are conserved throughout the process.[/bold green]\n"
    )
