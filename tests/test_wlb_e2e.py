...
from distca.mem import enable_memory_usage_logging, log_memory_usage, log_memory_usage_context
...
class MegatronE2eWorker(BaseMegatronE2eWorker):
    ...
    def forward_backward_batch(
        ...
        orig_fwd_backward_func = (
            wlbllm.megatron_patch.pp_schedules.forward_backward_pipelining_without_interleaving
        )
...
def init_megatron_e2e_test(
    ...
    print(
        f"[Rank {rank}] WLBLLM Communication groups initialized: {dp_rank=}, {pp_rank=}, {cp_rank=}, {tp_rank=}, {worker.as_rank=}, {worker.as_world_size=}"
    )
    return worker
...
from global_batch_provider import get_next_batch, setup_global_batch
...
def test(args):
    ...
    assert world_size % (tp_size * pp_size * cp_size) == 0, (
        f"world_size: {world_size} % (tp_size * pp_size * cp_size) == 0, {world_size} % {tp_size * pp_size * cp_size} != 0"
    )
    ...
    for sample_idx in range(max_sample_id):
        ...
        if ENABLE_BALANCED_FLOS_NO_DEFER:
            ...
            for i in range(num_microbatch):
                ...
                b = a[dp_rank * (batch_size_x2) : (dp_rank + 1) * (batch_size_x2)]
        ...
        assert len(balanced_seq_lens) == num_microbatch, (
            f"len(balanced_seq_lens) == num_microbatch, {len(balanced_seq_lens)} != {num_microbatch}"
        )
        ...
        for mb_idx, seq_lens in enumerate(balanced_seq_lens):
            ...
            print(
                f"🟡 balance_data_for_wlbllm[{mb_idx}]: doc_lens: {doc_lens}, seq_lens: {seq_lens}"
            )
            ...
            assert num_tokens_this_rank * cp_size == context_length, (
                f"num_tokens_this_rank * cp_size == context_length, {num_tokens_this_rank * cp_size} != {context_length}"
            )
        ...
        for repeat_idx in range(max_repeat_cnt + max_warmup_cnt):
            ...
            with torch.cuda.nvtx.range(
                f"wlbllm({config_name})[sample={sample_idx}][repeat={repeat_idx}]"
            ):
                ...
                print(
                    f"⚪ [Rank {rank}] [repeat {repeat_idx}] Finish running wlbllm: {duration_ms:.2f} ms"
                )
            ...
            time.sleep(1)
            log_memory_usage(
                f"forward_backward_batch:done(sample_id={sample_idx},repeat={repeat_idx})"
            )
