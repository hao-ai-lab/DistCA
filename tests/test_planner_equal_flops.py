...
from distca.planner.equal_flops import batch_to_items, plan_relocation
...
def test_planner_equal_flops():
    ...
    for item in expected_replanned_items:
        assert item in replanned_items, (
            f"item = {item} not in replanned_items: {expected_replanned_items = }\n{replanned_items = }"
        )
