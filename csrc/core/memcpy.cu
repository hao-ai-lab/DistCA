...
__global__ void memcpy_nvshmem_non_cp(uint8_t *tensor, uint8_t *nvshmem_buffer,
                                      const int64_t *seq_nvshmem_offset, const int64_t *seq_tokens,
                                      const int8_t *copy_seq_mask, const int64_t token_bytes,
  ...
  for (size_t token_idx = blockIdx.x; token_idx < total_num_tokens; token_idx += gridDim.x) {
    ...
    for (size_t i = threadIdx.x; i * sizeof(int4) < token_bytes; i += blockDim.x) {
...
__global__ void memcpy_nvshmem_cp(uint8_t *tensor, uint8_t *nvshmem_buffer, const int8_t *do_shard,
                                  const int64_t *seq_nvshmem_offset, const int64_t *seq_tokens,
                                  const int64_t token_bytes, const int32_t total_num_tokens,
                                  const int32_t max_num_cp, const int64_t num_seq) {
  ...
  for (size_t token_idx = blockIdx.x; token_idx < logical_num_tokens; token_idx += gridDim.x) {
    // NOTE: token_idx is a logical index on the (cp_id, seq_id, token_id)
    // order.
    while (token_idx >= cur_seq_end) {
      // Move to the next sequence
      cur_seq += 1;
    ...
    const size_t token_physical_offset_on_tensor =
        (TO_NVSHMEM ? (token_idx % total_num_tokens) : token_idx) * token_bytes;
...
template <typename T>
__global__ void
inplace_grad_sum_kernel(T *data, const int32_t *num_copies, const int64_t *copy_start_id,
                        const int64_t *seq_lens, const int64_t num_tokens,
                        // NOTE: unlike other kernels, hidden is not in terms of bytes,
                        // but instead in terms of elements
                        const int64_t hidden, const int64_t num_seq, const int64_t num_max_copies) {
  size_t seq_id = 0;
  ...
  for (size_t glob_id = blockIdx.x; glob_id < num_tokens; glob_id += gridDim.x) {
    ...
    for (int32_t copy_idx = 0; copy_idx < num_copies[seq_id]; ++copy_idx) {
      ...
      for (int h_vec = threadIdx.x; h_vec < hidden_vec_size; h_vec += blockDim.x) {
        ...
        // template. For float, you'd cast to float4. Let's assume a generic
        // cast. NOTE: This requires `T` to be compatible in size with int
        // components. A production kernel would often use template
        // specialization for types like float/half.
...
void launch_memcpy_non_cp(uint8_t *tensor, uint8_t *nvshmem_buffer,
                          const int64_t *seq_nvshmem_offset, const int64_t *seq_tokens,
                          const int8_t *copy_seq_mask, const int64_t token_bytes,
                          const int32_t total_num_tokens, const bool to_nvshmem,
                          cudaStream_t stream) {
  const int num_blocks = std::max(total_num_tokens / 32, 128);
  constexpr int WARP_SIZE = 32;
  int NUM_WARPS =
      std::min(10,       // at most 10 warps
  ...
  void *kernel = to_nvshmem
                     ? (copy_seq_mask != nullptr ? (void *)memcpy_nvshmem_non_cp<true, true>
                                                 : (void *)memcpy_nvshmem_non_cp<true, false>)
                     : ((void *)memcpy_nvshmem_non_cp<false, false>);
  CUDACHECK(cudaLaunchKernel(kernel, dimGrid, dimBlock, args, sharedMemory, stream));
}

void launch_memcpy_cp(uint8_t *tensor, uint8_t *nvshmem_buffer, const int8_t *do_shard,
                      const int64_t *seq_nvshmem_offset, const int64_t *seq_tokens,
                      const int64_t token_bytes, const int32_t total_num_tokens,
                      const int32_t max_num_cp, const int64_t num_seq, const bool to_nvshmem,
                      cudaStream_t stream) {
  ...
  if (to_nvshmem) {
    CUDACHECK(cudaLaunchKernel((void *)memcpy_nvshmem_cp<true>, dimGrid, dimBlock, args,
                               sharedMemory, stream));
  } else {
    CUDACHECK(cudaLaunchKernel((void *)memcpy_nvshmem_cp<false>, dimGrid, dimBlock, args,
                               sharedMemory, stream));
  }
}

template <typename T>
void launch_inplace_grad_sum(T *data, const int32_t *num_copies, const int64_t *copy_start_id,
                             const int64_t *seq_lens, const int64_t num_tokens,
                             // NOTE: unlike other kernels, hidden is not in
                             // terms of bytes, but instead in terms of elements
                             const int64_t hidden, const int64_t num_seq,
                             const int64_t num_max_copies, cudaStream_t stream) {
  ...
  CUDACHECK(cudaLaunchKernel((void *)inplace_grad_sum_kernel<T>, dimGrid, dimBlock, args,
                             sharedMemory, stream));
}
...
#define INSTANTIATE_INPLACE_GRAD_SUM(T)                                                            \
  template void launch_inplace_grad_sum<T>(                                                        \
      T * data, const int32_t *num_copies, const int64_t *copy_start_id, const int64_t *seq_lens,  \
      const int64_t num_tokens, const int64_t hidden, const int64_t num_seq,                       \
      const int64_t num_max_copies, cudaStream_t stream);
