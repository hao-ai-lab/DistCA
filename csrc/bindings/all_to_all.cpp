...
fptr_t create_fast_a2a_dispatch_helper(int64_t rank, int64_t local_rank, int64_t world_size,
                                       int64_t buffer_size) {
  auto *ptr = new FastA2aDispatchHelper(rank, local_rank, world_size, buffer_size);
  return (fptr_t)ptr;
}

void destroy_fast_a2a_dispatch_helper(fptr_t fptr) { delete (FastA2aDispatchHelper *)fptr; }

void update_buffer_size(fptr_t fptr, int64_t target_size) {
  auto *dispatch_helper = (FastA2aDispatchHelper *)fptr;
  dispatch_helper->update_buffer_size(target_size);
}

void _fast_a2a_memcpy_non_cp_core(at::Tensor &tensor, uint8_t *buffer_ptr,
  ...
  launch_memcpy_non_cp(tensor_ptr, buffer_ptr, seq_nvshmem_offset.const_data_ptr<int64_t>(),
                       seq_tokens.const_data_ptr<int64_t>(),
                       copy_seq_mask.has_value() ? copy_seq_mask.value().const_data_ptr<int8_t>()
                                                 : nullptr,
                       token_bytes, num_tokens, to_nvshmem, stream);
}

void fast_a2a_memcpy_non_cp(fptr_t fptr, at::Tensor &tensor, const at::Tensor &seq_nvshmem_offset,
                            const at::Tensor &seq_tokens,
                            const std::optional<at::Tensor> &copy_seq_mask, const bool to_nvshmem) {
  ...
  _fast_a2a_memcpy_non_cp_core(tensor, buffer_ptr, seq_nvshmem_offset, seq_tokens, copy_seq_mask,
                               to_nvshmem);
}

void fast_a2a_memcpy_non_cp_debug(at::Tensor &tensor, const at::Tensor &seq_nvshmem_offset,
                                  const at::Tensor &seq_tokens,
                                  const std::optional<at::Tensor> &copy_seq_mask,
                                  const bool to_nvshmem, at::Tensor &buffer) {
  uint8_t *buffer_ptr = (uint8_t *)buffer.data_ptr();
  _fast_a2a_memcpy_non_cp_core(tensor, buffer_ptr, seq_nvshmem_offset, seq_tokens, copy_seq_mask,
                               to_nvshmem);
}

void _fast_a2a_memcpy_cp_core(at::Tensor &tensor, uint8_t *buffer_ptr, const at::Tensor &do_shard,
                              const at::Tensor &seq_nvshmem_offset, const at::Tensor &seq_tokens,
                              const bool to_nvshmem) {
  if (to_nvshmem) {
    _CHECK_TENSOR(2, tensor);
  } else {
  ...
  const int64_t token_bytes =
      ((to_nvshmem ? tensor.size(1) : tensor.size(2)) * tensor.element_size());
  ...
  TORCH_CHECK(seq_tokens.size(0) == num_seq, "seq_tokens must be of dimension (num_sequence)");
  ...
  at::cuda::OptionalCUDAGuard const device_guard(device_of(tensor));
  cudaStream_t stream = c10::cuda::getCurrentCUDAStream().stream();
  launch_memcpy_cp(tensor_ptr, buffer_ptr, do_shard.const_data_ptr<int8_t>(),
                   seq_nvshmem_offset.const_data_ptr<int64_t>(),
                   seq_tokens.const_data_ptr<int64_t>(), token_bytes, num_tokens, cp_degree,
                   num_seq, to_nvshmem, stream);
}

void fast_a2a_memcpy_cp(fptr_t fptr, at::Tensor &tensor, const at::Tensor &do_shard,
                        const at::Tensor &seq_nvshmem_offset, const at::Tensor &seq_tokens,
                        const bool to_nvshmem) {
  // This is the non-buffer branch.
  uint8_t *buffer_ptr;
  ...
  _fast_a2a_memcpy_cp_core(tensor, buffer_ptr, do_shard, seq_nvshmem_offset, seq_tokens,
                           to_nvshmem);
}

void fast_a2a_grad_acc_pre_memcpy_cp(at::Tensor &tensor, const at::Tensor num_copies,
                                     const at::Tensor copy_start_id, const at::Tensor seq_lens) {
  _CHECK_TENSOR(2, tensor);
  ...
  TORCH_CHECK(seq_lens.size(0) == num_seqs, "seq_lens must be of shape (num_sequence,)");
  ...
  if (tensor.dtype() == at::kFloat) {
    launch_inplace_grad_sum<float>((float *)(tensor.data_ptr()), num_copies.data_ptr<int32_t>(),
                                   copy_start_id.data_ptr<int64_t>(), seq_lens.data_ptr<int64_t>(),
                                   num_tokens, hidden, num_seqs, num_max_copies,
                                   c10::cuda::getCurrentCUDAStream().stream());
  } else if (tensor.dtype() == at::kHalf) {
    launch_inplace_grad_sum<half>((half *)(tensor.data_ptr()), num_copies.data_ptr<int32_t>(),
                                  copy_start_id.data_ptr<int64_t>(), seq_lens.data_ptr<int64_t>(),
                                  num_tokens, hidden, num_seqs, num_max_copies,
                                  c10::cuda::getCurrentCUDAStream().stream());
  } else if (tensor.dtype() == at::kBFloat16) {
    launch_inplace_grad_sum<nv_bfloat16>(
        (nv_bfloat16 *)(tensor.data_ptr()), num_copies.data_ptr<int32_t>(),
        copy_start_id.data_ptr<int64_t>(), seq_lens.data_ptr<int64_t>(), num_tokens, hidden,
        num_seqs, num_max_copies, c10::cuda::getCurrentCUDAStream().stream());
  } else {
    TORCH_CHECK(false, "Only fp32, fp16 and bf16 inputs are supported");
  }
}

void fast_a2a_memcpy_cp_debug(at::Tensor &tensor, const at::Tensor &do_shard,
                              const at::Tensor &seq_nvshmem_offset, const at::Tensor &seq_tokens,
                              const bool to_nvshmem, at::Tensor &buffer) {
  uint8_t *buffer_ptr = (uint8_t *)buffer.data_ptr();
  _fast_a2a_memcpy_cp_core(tensor, buffer_ptr, do_shard, seq_nvshmem_offset, seq_tokens,
...
void fast_a2a(fptr_t fptr, const at::Tensor &sender_send_disp_tensor,
              const at::Tensor &sender_transfer_sz_tensor,
              const at::Tensor &sender_recv_disp_tensor,
              const at::Tensor &recver_transfer_sz_tensor, int64_t my_rank_send_offset,
              int64_t my_rank_recv_offset, int64_t my_rank_send_sz, bool separate_send_recv) {
  auto dispatch_helper = (FastA2aDispatchHelper *)fptr;
  ...
  cudaStream_t stream = c10::cuda::getCurrentCUDAStream().stream();
  internode_transfer_params_t param{.sender_send_disp = sender_send_disp,
                                    .sender_transfer_sz = sender_transfer_sz,
                                    .sender_recv_disp = sender_recv_disp,
                                    .recver_transfer_sz = recver_transfer_sz};

  launch_alltoallv(dispatch_helper->_rank, 1, world_size, &dispatch_helper->buffer, &param,
                   my_rank_send_offset, my_rank_recv_offset, my_rank_send_sz, stream,
                   dispatch_helper->_buffer_size, separate_send_recv);
}

void release_buffer(fptr_t fptr) {
  auto dispatch_helper = (FastA2aDispatchHelper *)fptr;
  cudaStream_t stream = c10::cuda::getCurrentCUDAStream().stream();
  launch_buffer_availability_kernel(dispatch_helper->_rank, 1, dispatch_helper->_world_size,
                                    &dispatch_helper->buffer,
                                    true, // is_release
                                    stream);
}

void wait_and_consume_buffer(fptr_t fptr) {
  auto dispatch_helper = (FastA2aDispatchHelper *)fptr;
  cudaStream_t stream = c10::cuda::getCurrentCUDAStream().stream();
  launch_buffer_availability_kernel(dispatch_helper->_rank, 1, dispatch_helper->_world_size,
                                    &dispatch_helper->buffer,
...
void _debug_nvshmem_buffer(fptr_t fptr, bool get_send, bool get_recv, bool get_signal,
                           at::Tensor &target) {
  if (get_send) {
  ...
  if (get_send || get_recv) {
    ...
    uint8_t *dst_ptr = (uint8_t *)(target.data_ptr());
    uint8_t *src_ptr =
        get_send ? dispatch_helper->buffer.send_buffer : dispatch_helper->buffer.recv_buffer;
    ...
    cudaMemcpy(dst_ptr, dispatch_helper->buffer.sync_signal, size * sizeof(uint64_t),
